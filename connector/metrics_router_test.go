...
func fuzzMetrics(numIDs, numCons, numMetrics int) func(*testing.T) {
	return func(t *testing.T) {
		...
		allCons := make([]consumermetrics.Metrics, 0, numCons)
		allConsMap := make(map[component.ID]consumermetrics.Metrics)
...
func TestMetricsRouterConsumers(t *testing.T) {
	...
	r := NewMetricsRouter(map[component.ID]consumermetrics.Metrics{fooID: foo, barID: bar})
