...
func TestCount(t *testing.T) {
	...
	spansCounter, err := f.CreateTracesToMetrics(ctx, set, cfg, countsSink)
	assert.NoError(t, err)
	assert.NotNil(t, spansCounter)

	metricsCounter, err := f.CreateMetricsToMetrics(ctx, set, cfg, countsSink)
	assert.NoError(t, err)
	assert.NotNil(t, metricsCounter)

	logsCounter, err := f.CreateLogsToMetrics(ctx, set, cfg, countsSink)
	assert.NoError(t, err)
	...
	assert.NoError(t, spansCounter.ConsumeTraces(ctx, dummySpans(spanCounts)))

	metricCounts := []int64{2, 5, 1, 10}
	assert.NoError(t, metricsCounter.ConsumeMetrics(ctx, dummyMetrics(metricCounts)))

	logCounts := []int64{1, 1, 2, 3, 5, 8, 13}
	assert.NoError(t, logsCounter.ConsumeLogs(ctx, dummyLogs(logCounts)))
