...
func (mapping *exponentialMapping) scanPositive(buckets pdata.Buckets, boundaries []float64, counts []uint64) {
	bucketNumber := int64(0)

	for position, bound := range boundaries {
		index := mapping.MapToIndex(bound)
		...
		if bucketNumber+buckets.Offset() == index {
			...
			// the lowerBound.
			lowerBound := mapping.lowerBoundary(index)
			upperBound := mapping.upperBoundary(index)
...
func (mapping *exponentialMapping) scanNegative(buckets pdata.Buckets, boundaries []float64, counts []uint64) {
	bucketNumber := int64(0)
...
func toExplicitPoint(input pdata.ExponentialHistogramDataPoint, boundaries []float64) pdata.HistogramDataPoint {
	...
	if zeroCrosser == 0 {
		...
		mapping.scanPositive(input.Positive(), boundaries, counts)
		...
		mapping.scanNegative(input.Negative(), boundaries, counts)

	} else if boundaries[zeroCrosser-1] == 0 {
		// We have a boundary at zero as in (-1, 0], (0, 1]
		counts[zeroCrosser-1] += input.ZeroCount()

		mapping.scanNegative(input.Negative(), boundaries[0:zeroCrosser-1], counts[0:zeroCrosser])
		mapping.scanPositive(input.Positive(), boundaries[zeroCrosser:], counts[zeroCrosser:])
	} else {
		// We have a bucket that spans zero.
		counts[zeroCrosser] += input.ZeroCount()

		mapping.scanNegative(input.Negative(), boundaries[0:zeroCrosser], counts[0:zeroCrosser+1])
		mapping.scanPositive(input.Positive(), boundaries[zeroCrosser:], counts[zeroCrosser:])
	}
	return r
}
