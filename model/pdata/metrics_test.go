...
func TestMetricCount(t *testing.T) {
	...
	rms.At(2).InstrumentationLibraryMetrics().AppendEmpty().Metrics().AppendEmptyN(5)
...
func TestMetricAndDataPointCount(t *testing.T) {
	...
	ilms.At(0).Metrics().AppendEmptyN(1)
	dps = md.DataPointCount()
	assert.EqualValues(t, 0, dps)
	ilms.At(0).Metrics().At(0).SetDataType(MetricDataTypeIntSum)
	intSum := ilms.At(0).Metrics().At(0).IntSum()
	intSum.DataPoints().AppendEmptyN(3)
	assert.EqualValues(t, 3, md.DataPointCount())

	md = NewMetrics()
	rms = md.ResourceMetrics()
	rms.AppendEmptyN(3)
	rms.At(0).InstrumentationLibraryMetrics().AppendEmptyN(1)
	rms.At(0).InstrumentationLibraryMetrics().At(0).Metrics().AppendEmptyN(1)
	rms.At(1).InstrumentationLibraryMetrics().AppendEmptyN(1)
	rms.At(2).InstrumentationLibraryMetrics().AppendEmptyN(1)
	ilms = rms.At(2).InstrumentationLibraryMetrics()
	ilms.AppendEmptyN(1)
	ilms.At(0).Metrics().AppendEmptyN(5)
...
func TestOtlpToFromInternalIntGaugeMutating(t *testing.T) {
	...
	int64DataPoints.AppendEmptyN(1)
...
func TestOtlpToFromInternalDoubleSumMutating(t *testing.T) {
	...
	doubleDataPoints := metric.DoubleSum().DataPoints()
	metric.DoubleSum().SetAggregationTemporality(AggregationTemporalityCumulative)
	doubleDataPoints.AppendEmptyN(1)
...
func TestOtlpToFromInternalHistogramMutating(t *testing.T) {
	...
	histogramDataPoints.AppendEmptyN(1)
