...
type oneOfField struct {
	...
	originFieldName  string
	// TODO: See if we can use a unified type accessor name.
...
func (of *oneOfField) generateAccessorsTest(ms baseStruct, sb *strings.Builder) {
	for _, v := range of.values {
		v.generateAccessorsTest(ms.(*messageValueStruct), of, sb)
		sb.WriteString("\n")
	}
}
...
type oneOfValue interface {
	...
	generateAccessorsTest(ms baseStruct, of *oneOfField, sb *strings.Builder)
...
func (opv *oneOfPrimitiveValue) generateAccessorsTest(ms baseStruct, of *oneOfField, sb *strings.Builder) {
	sb.WriteString(os.Expand(accessorsPrimitiveTestTemplate, func(name string) string {
		switch name {
		case "structName":
			return ms.getName()
...
func (omv *oneOfMessageValue) generateAccessors(ms baseStruct, of *oneOfField, sb *strings.Builder) {
	sb.WriteString(os.Expand(accessorsOneOfMessageTemplate, func(name string) string {
		...
		case "typeName":
			return of.typeName + omv.fieldType
		default:
			panic(name)
		}
	}))
...
func (omv *oneOfMessageValue) generateAccessorsTest(ms baseStruct, of *oneOfField, sb *strings.Builder) {
...
func (omv *oneOfMessageValue) generateSetWithTestValue(of *oneOfField, sb *strings.Builder) {
	sb.WriteString("tv.Set" + of.typeAccessor + "(" + of.typeName + omv.fieldType + ")\n")
