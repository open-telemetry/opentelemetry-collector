...
func TestAttributeValue(t *testing.T) {
	...
	assert.EqualValues(t, "abc", val)

	v = NewAttributeValueInt(123)
	assert.EqualValues(t, AttributeValueTypeInt, v.Type())
	intValue, err := v.IntValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 123, intValue)

	v = NewAttributeValueDouble(3.4)
	assert.EqualValues(t, AttributeValueTypeDouble, v.Type())
	doubleValue, err := v.DoubleValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 3.4, doubleValue)

	v = NewAttributeValueBool(true)
	assert.EqualValues(t, AttributeValueTypeBool, v.Type())
	boolValue, err := v.BoolValue()
	assert.NoError(t, err)
	...
	assert.NoError(t, err)
	assert.EqualValues(t, "abc", stringValue)
	...
	assert.EqualValues(t, AttributeValueTypeBytes, v.Type())
	bytesValue, err = v.BytesValue()
	assert.NoError(t, err)
	assert.EqualValues(t, bytesValue, bytesValue)
}
...
func TestAttributeValueMap(t *testing.T) {
	...
	val, err := m1.MapValue()
	assert.NoError(t, err)
	assert.Equal(t, NewAttributeMap(), val)
	val, err = m1.MapValue()
	assert.NoError(t, err)
	...
	val, err = m1.MapValue()
	assert.NoError(t, err)
	val.InsertDouble("double_key", 123)
	val, err = m1.MapValue()
	assert.NoError(t, err)
	assert.Equal(t, 1, val.Len())
	val, err = m1.MapValue()
	assert.NoError(t, err)
	got, exists := val.Get("double_key")
	assert.True(t, exists)
	assert.Equal(t, NewAttributeValueDouble(123), got)
	...
	val, err = m2.MapValue()
	assert.NoError(t, err)
	val.UpsertString("key_in_child", "somestr")
	val, err = m2.MapValue()
	assert.NoError(t, err)
	assert.Equal(t, 1, val.Len())
	val, err = m2.MapValue()
	assert.NoError(t, err)
	got, exists = val.Get("key_in_child")
	assert.True(t, exists)
	assert.Equal(t, NewAttributeValueString("somestr"), got)
	...
	val, err = m1.MapValue()
	assert.NoError(t, err)
	val.Insert("child_map", m2)
	val, err = m1.MapValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 2, val.Len())
	val, err = m1.MapValue()
	assert.NoError(t, err)
	got, exists = val.Get("double_key")
	assert.True(t, exists)
	...
	val, err = m2.MapValue()
	assert.NoError(t, err)
	val.UpdateString("key_in_child", "somestr2")
	val, err = m2.MapValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 1, val.Len())
	val, err = m2.MapValue()
	assert.NoError(t, err)
	got, exists = val.Get("key_in_child")
	assert.True(t, exists)
	...
	val, err = m1.MapValue()
	assert.NoError(t, err)
	childMap, childMapExists := val.Get("child_map")
	require.True(t, childMapExists)
	val, err = childMap.MapValue()
	assert.NoError(t, err)
	got, exists = val.Get("key_in_child")
	require.True(t, exists)
	assert.Equal(t, NewAttributeValueString("somestr"), got)
	...
	val, err = childMap.MapValue()
	assert.NoError(t, err)
	val.UpdateString("key_in_child", "somestr3")
	val, err = childMap.MapValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 1, val.Len())
	val, err = childMap.MapValue()
	...
	val, err = m1.MapValue()
	assert.NoError(t, err)
	_, exists = val.Get("double_key")
	assert.False(t, exists)

	val, err = m1.MapValue()
	assert.NoError(t, err)
	deleted = val.Delete("child_map")
	assert.True(t, deleted)
	val, err = m1.MapValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 0, val.Len())
	val, err = m1.MapValue()
	assert.NoError(t, err)
	_, exists = val.Get("child_map")
	assert.False(t, exists)
}
...
func TestNilOrigSetAttributeValue(t *testing.T) {
	...
	stringValue, err := av.StringValue()
	assert.NoError(t, err)
	assert.EqualValues(t, "abc", stringValue)

	av = NewAttributeValueEmpty()
	av.SetIntValue(123)
	intValue, err := av.IntValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 123, intValue)

	av = NewAttributeValueEmpty()
	av.SetBoolValue(true)
	boolValue, err := av.BoolValue()
	assert.NoError(t, err)
	assert.True(t, boolValue)

	av = NewAttributeValueEmpty()
	av.SetDoubleValue(1.23)
	doubleValue, err := av.DoubleValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 1.23, doubleValue)

	av = NewAttributeValueEmpty()
	av.SetBytesValue([]byte{1, 2, 3})
	bytesValue, err := av.BytesValue()
	assert.NoError(t, err)
	assert.Equal(t, []byte{1, 2, 3}, bytesValue)
}
...
func TestAttributeValueEqual(t *testing.T) {
	...
	sliceValue, err := av1.SliceValue()
	assert.NoError(t, err)
	sliceValue.AppendEmpty().SetIntValue(123)
	assert.False(t, av1.Equal(av2))
	assert.False(t, av2.Equal(av1))

	av2 = NewAttributeValueArray()
	sliceValue, err = av2.SliceValue()
	assert.NoError(t, err)
	sliceValue.AppendEmpty().SetDoubleValue(123)
	assert.False(t, av1.Equal(av2))

	sliceValue, err = av2.SliceValue()
	assert.NoError(t, err)
	NewAttributeValueInt(123).CopyTo(sliceValue.At(0))
	assert.True(t, av1.Equal(av2))

	sliceValue, err = av2.SliceValue()
	assert.NoError(t, err)
	av1.CopyTo(sliceValue.AppendEmpty())
	assert.False(t, av1.Equal(av2))

	av1 = NewAttributeValueMap()
	mapValue, err := av1.MapValue()
	assert.NoError(t, err)
	mapValue.UpsertString("foo", "bar")
	assert.False(t, av1.Equal(av2))
	assert.False(t, av2.Equal(av1))

	av2 = NewAttributeValueMap()
	mapValue, err = av2.MapValue()
	assert.NoError(t, err)
	mapValue.UpsertString("foo", "bar")
	assert.True(t, av1.Equal(av2))
...
func TestAttributeMapWithEmpty(t *testing.T) {
	...
	stringValue, err := val.StringValue()
	assert.NoError(t, err)
	assert.EqualValues(t, "test_value", stringValue)
	...
	stringValue, err = val.StringValue()
	assert.NoError(t, err)
	assert.EqualValues(t, "other_value", stringValue)
	...
	doubleValue, err := val.DoubleValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 1.23, doubleValue)
	...
	bytesValue, err := val.BytesValue()
	assert.NoError(t, err)
	assert.EqualValues(t, []byte{1, 2, 3}, bytesValue)
	...
	stringValue, err = val.StringValue()
	assert.NoError(t, err)
	assert.EqualValues(t, "yet_another_value", stringValue)
	...
	intValue, err = val.IntValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 456, intValue)
	...
	doubleValue, err = val.DoubleValue()
	assert.NoError(t, err)
	assert.EqualValues(t, 4.56, doubleValue)
	...
	bytesValue, err = val.BytesValue()
	assert.NoError(t, err)
	assert.EqualValues(t, []byte{4, 5, 6}, bytesValue)
	...
	bytesValue, err = val.BytesValue()
	assert.NoError(t, err)
	assert.EqualValues(t, []byte{7, 8, 9}, bytesValue)
	...
	bytesValue, err = val.BytesValue()
	assert.NoError(t, err)
	assert.EqualValues(t, []byte{1}, bytesValue)
...
func TestAttributeValueArray(t *testing.T) {
	a1 := NewAttributeValueArray()
	assert.EqualValues(t, AttributeValueTypeArray, a1.Type())
	a1Val, err := a1.SliceValue()
	assert.NoError(t, err)
	assert.EqualValues(t, NewAttributeValueSlice(), a1Val)
	assert.EqualValues(t, 0, a1Val.Len())

	a1Val.AppendEmpty().SetDoubleValue(123)
	assert.EqualValues(t, 1, a1Val.Len())
	assert.EqualValues(t, NewAttributeValueDouble(123), a1Val.At(0))
	// Create a second array.
	a2 := NewAttributeValueArray()
	a2Val, err := a2.SliceValue()
	assert.EqualValues(t, 0, a2Val.Len())

	a2Val.AppendEmpty().SetStringValue("somestr")
	assert.NoError(t, err)
	assert.EqualValues(t, 1, a2Val.Len())
	assert.EqualValues(t, NewAttributeValueString("somestr"), a2Val.At(0))

	// Insert the second array as a child.
	a2.CopyTo(a1Val.AppendEmpty())
	assert.EqualValues(t, 2, a1Val.Len())
	assert.EqualValues(t, NewAttributeValueDouble(123), a1Val.At(0))
	assert.EqualValues(t, a2, a1Val.At(1))

	// Check that the array was correctly inserted.
	childArray := a1Val.At(1)
	sliceValue, err := childArray.SliceValue()
	assert.NoError(t, err)
	assert.EqualValues(t, AttributeValueTypeArray, childArray.Type())
	assert.EqualValues(t, 1, sliceValue.Len())

	v := sliceValue.At(0)
	assert.EqualValues(t, AttributeValueTypeString, v.Type())
	stringValue, err := v.StringValue()
	assert.NoError(t, err)
	assert.EqualValues(t, "somestr", stringValue)
}
...
func TestAsRaw(t *testing.T) {
	...
	val, err := arr.SliceValue()
	assert.NoError(t, err)
	val.AppendEmpty().SetBoolValue(false)
	val.AppendEmpty().SetBytesValue([]byte("test"))
	val.AppendEmpty().SetDoubleValue(12.9)
	val.AppendEmpty().SetIntValue(91)
	val.AppendEmpty().SetStringValue("another string")
...
func constructTestAttributeSubmap() AttributeValue {
	...
	mapValue, _ := value.MapValue()
	mapValue.UpsertString("keyOne", "valOne")
	mapValue.UpsertString("keyTwo", "valTwo")
	return value
}

func constructTestAttributeSubarray() AttributeValue {
	value := NewAttributeValueArray()
	sliceValue, _ := value.SliceValue()
	sliceValue.AppendEmpty().SetStringValue("strOne")
	sliceValue.AppendEmpty().SetStringValue("strTwo")
	return value
}
