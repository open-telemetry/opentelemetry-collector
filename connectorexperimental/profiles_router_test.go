...
type mutatingProfilesSink struct {
	*consumerexperimentaltest.ProfilesSink
}
...
func fuzzProfiles(numIDs, numCons, numProfiles int) func(*testing.T) {
	return func(t *testing.T) {
		...
		allCons := make([]consumerexperimental.Profiles, 0, numCons)
		allConsMap := make(map[component.ID]consumerexperimental.Profiles)
		...
		for i := 0; i < numCons; i++ {
			...
			if (numCons+numProfiles+i)%4 == 0 {
				allCons = append(allCons, &mutatingProfilesSink{ProfilesSink: new(consumerexperimentaltest.ProfilesSink)})
			} else {
				allCons = append(allCons, new(consumerexperimentaltest.ProfilesSink))
			}
		...
		for i := 0; i < numProfiles; i++ {
			...
			for id := range expected {
				...
				case *consumerexperimentaltest.ProfilesSink:
...
func TestProfilesRouterConsumer(t *testing.T) {
	...
	foo := new(consumerexperimentaltest.ProfilesSink)
	bar := new(consumerexperimentaltest.ProfilesSink)
	r := NewProfilesRouter(map[component.ID]consumerexperimental.Profiles{fooID: foo, barID: bar})
