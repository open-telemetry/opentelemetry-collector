...
// Name is the component name as it is used in the config.
type Name struct {
	value string
}
...
// MarshalText marshals the Name value.
...
// NewName creates a name. It returns an error if the name is invalid.
// A name must
// - have at least one character,
...
func NewName(n string) (Name, error) {
	...
	if !nameRegexp.MatchString(n) {
		return Name{}, fmt.Errorf("invalid character(s) in name %q", n)
	}
	return Name{value: n}, nil
}

// MustNewName creates a name. It panics if the name is invalid.
// A name must
// - have at least one character,
// - start with an ASCII alphanumeric character and
// - can only contain ASCII alphanumeric characters, '_', and '-'.
func MustNewName(strName string) Name {
	n, err := NewName(strName)
	if err != nil {
		panic(err)
	}
	return n
