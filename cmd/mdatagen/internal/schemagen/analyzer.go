...
func NewPackageAnalyzer(dir string) *PackageAnalyzer {
	return &PackageAnalyzer{dir: dir}
}
...
func (a *PackageAnalyzer) extractFieldInfo(pkg *packages.Package, field *types.Var, tag string) *FieldInfo {
	...
	// Parse struct tags
	jsonName, omitempty := parseTag(tag)
	if jsonName == "-" {
	...
	// Check if field is optional (configoptional.Optional[T])
	...
	info := &FieldInfo{
		...
		Required:    !omitempty && !embedded && !isOptional,
		Embedded:    embedded,
	}
...
// parseTag parses struct tags to find the mapstructure name.
