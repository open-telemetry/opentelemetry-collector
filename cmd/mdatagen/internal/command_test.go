...
`,
		...
		{
			name: "readme with status for nested extension path",
			markdown: `# Some component
...
Some info about a component
`,
			outputFile:     "readme_with_status_nested_extension.md",
			componentClass: "extension",
			distros:        []string{"contrib"},
			packageName:    "github.com/open-telemetry/opentelemetry-collector-contrib/extension/storage/filestorage",
			importRootPath: "github.com/open-telemetry/opentelemetry-collector-contrib",
			shortFolder:    "filestorage",
		},
		{
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			shortFolder := tt.shortFolder
			if shortFolder == "" {
				shortFolder = "foo"
			}
			importRootPath := tt.importRootPath
			if importRootPath == "" {
			...
			err := inlineReplace("templates/readme.md.tmpl", readmeFile, md, statusStart, statusEnd, "metadata", importRootPath)
			require.NoError(t, err)
