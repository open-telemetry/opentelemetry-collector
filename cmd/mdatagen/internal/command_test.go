...
`,
		...
		{
			name: "readme with nested issue label",
			markdown: `# Some component
...
Some info about a component
`,
			outputFile:     "readme_with_nested_issue_label.md",
			componentClass: "extension",
			shortFolder:    "foo",
			packageName:    "go.opentelemetry.io/collector-contrib/extension/storage/foo",
			distros:        []string{"contrib"},
		},
		{
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shortFolder != "" {
				md.ShortFolderName = tt.shortFolder
			}
			tmpdir := t.TempDir()
