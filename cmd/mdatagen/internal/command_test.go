...
placeholder
<!-- end config autogenerated section -->`), 0o600))
			...
			if tt.wantConfigDocGenerated {
				contents, err = os.ReadFile(filepath.Clean(filepath.Join(tmpdir, "README.md")))
				require.NoError(t, err)
				require.Contains(t, string(contents), configDocStart)
				require.Contains(t, string(contents), configDocEnd)
				require.NotContains(t, string(contents), "placeholder")
			}
