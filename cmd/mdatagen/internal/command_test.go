...
`,
		...
		{
			name: "readme with nested component path",
			markdown: `# Some component
...
Some info about a component
`,
			outputFile:     "readme_with_nested_component_path.md",
			componentClass: "extension",
			distros:        []string{"contrib"},
			githubProject:  "open-telemetry/opentelemetry-collector",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			packageName := "go.opentelemetry.io/collector/extension/storage/filestorage"
			if tt.name != "readme with nested component path" {
				packageName = ""
			}
			md := Metadata{
