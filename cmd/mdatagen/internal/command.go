...
func run(ymlPath string) error {
	...
	if md.Status != nil {
		...
		if _, err = os.Stat(filepath.Join(ymlDir, "README.md")); err == nil {
			...
			if md.Config != nil {
				err = inlineReplace(
					filepath.Join(tmplDir, "config_doc.md.tmpl"),
					filepath.Join(ymlDir, "README.md"),
					md, configDocStart, configDocEnd, md.GeneratedPackageName, importRootPath)
				if err != nil {
					return err
				}
			}
...
func getTemplateFuncMap(md Metadata, importRootPath string) template.FuncMap {
	return template.FuncMap{
		...
		"cfgPropDocs": func(cfg *cfggen.ConfigMetadata) []cfggen.PropDoc {
			return cfggen.ExtractPropDocs(cfg)
		},
