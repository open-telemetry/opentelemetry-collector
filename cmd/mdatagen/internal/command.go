...
func NewCommand() (*cobra.Command, error) {
	...
	rootCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			generatedPackageName, err := cmd.Flags().GetString(packageNameFlag)
			if err != nil {
				generatedPackageName = "metadata"
			}
			return run(args[0], generatedPackageName)
		},
	}
	rootCmd.Flags().String(packageNameFlag, "metadata", "the name of the package for mdatagen to generate")
	return rootCmd, nil
}

func run(ymlPath string, generatedPackageName string) error {
	if ymlPath == "" {
	...
	codeDir := filepath.Join(ymlDir, "internal", generatedPackageName)
	...
	if md.Status != nil {
		...
		if _, err = os.Stat(filepath.Join(ymlDir, "README.md")); err == nil {
			if err = inlineReplace(
				...
				md, statusStart, statusEnd, generatedPackageName); err != nil {
				return err
			}
		}
	}
	...
	for tmpl, dst := range toGenerate {
		if err = generateFile(tmpl, dst, md, generatedPackageName); err != nil {
			return err
		}
	}
