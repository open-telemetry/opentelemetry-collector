...
type Metadata struct {
	...
	// Logs that can be emitted by the component.
	Logs map[LogName]Log `mapstructure:"logs"`
...
func (md *Metadata) Validate() error {
	...
	if err := md.validateMetricsAndLogs(); err != nil {
		errs = errors.Join(errs, err)
	}
...
func (md *Metadata) validateMetricsAndLogs() error {
	var errs error
	...
	errs = errors.Join(errs,
		...
		validateLogs(md.Logs, md.Attributes, usedAttrs),
...
func validateLogs(logs map[LogName]Log, attributes map[AttributeName]Attribute, usedAttrs map[AttributeName]bool) error {
	var errs error
	for mn, m := range logs {
		if err := m.validate(); err != nil {
			errs = errors.Join(errs, fmt.Errorf(`log "%v": %w`, mn, err))
			continue
		}
		unknownAttrs := make([]AttributeName, 0, len(m.Attributes))
		for _, attr := range m.Attributes {
		...
		if len(unknownAttrs) > 0 {
			errs = errors.Join(errs, fmt.Errorf(`log "%v" refers to undefined attributes: %v`, mn, unknownAttrs))
		}
	}
	return errs
}
