...
func newTestBuilder(t *testing.T, variation string) (*TelemetryBuilder, *componenttest.Telemetry) {
	t.Helper()
	tt := componenttest.NewTelemetry()
	t.Cleanup(func() { require.NoError(t, tt.Shutdown(context.Background())) })

	tb, err := NewTelemetryBuilder(tt.NewTelemetrySettings(), variation)
...
func TestNameVariation(t *testing.T) {
	tests := []struct {
		name      string
		variation string
		expected  string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			tb, tt := newTestBuilder(t, tc.variation)
			assertMetricName(t, tb, tt, tc.expected)
		})
	}
}

// TestNameVariation_Required verifies that NewTelemetryBuilder rejects an
// empty variation when allow_name_variation is set.
func TestNameVariation_Required(t *testing.T) {
	tt := componenttest.NewTelemetry()
	t.Cleanup(func() { require.NoError(t, tt.Shutdown(context.Background())) })

	_, err := NewTelemetryBuilder(tt.NewTelemetrySettings(), "")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "variation is required")
}
