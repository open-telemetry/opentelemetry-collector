...
| ------- | ---- | ------- | -------- | ----------- |
| `collection_interval` | duration | 1m | no | CollectionInterval sets how frequently the scraper should be called and used as the context timeout to ensure that scrapers don't exceed the interval. |
| `initial_delay` | duration | 1s | no | InitialDelay sets the initial start delay for the scraper, any non positive value is assumed to be immediately. |
...
| `timeout` | duration | 0 | no | Timeout is an optional value used to set scraper's context deadline. |
