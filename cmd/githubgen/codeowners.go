...
#####################################################
#
# List of components in OpenTelemetry Collector
...
#####################################################
#
# List of codeowners for OpenTelemetry Collector
#
#####################################################
#
# Learn about membership in OpenTelemetry community:
	...
	for _, key := range data.folders {
		...
		for stability := range m.Status.Stability {
			if stability == unmaintainedStatus {
				...
				unmaintainedCodeowners += fmt.Sprintf("%s/%s @open-telemetry/collector-approvers \n", key, strings.Repeat(" ", data.maxLength-len(key)))
				continue LOOP
			}
		...
		if m.Status.Codeowners != nil {
			...
			codeowners += fmt.Sprintf("%s/%s @open-telemetry/collector-approvers%s\n", key, strings.Repeat(" ", data.maxLength-len(key)), owners)
		}
	}
