...
const (
	...
	skipGenerateSchemaFlag     = "skip-generate-schema"
	ldflagsFlag                = "ldflags"
	gcflagsFlag                = "gcflags"
...
func initFlags(flags *flag.FlagSet) error {
	...
	flags.Bool(skipGenerateSchemaFlag, false, "Whether builder should skip generating JSON schemas for component configs (default false)")
...
func applyFlags(flags *flag.FlagSet, cfg *builder.Config) error {
	...
	errs = multierr.Append(errs, err)
	cfg.SkipGenerateSchema, err = flags.GetBool(skipGenerateSchemaFlag)
	errs = multierr.Append(errs, err)
