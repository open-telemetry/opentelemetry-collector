...
func TestUnmarshal_TelemetryOtelConf(t *testing.T) {
	for _, tk := range testKinds {
		t.Run(tk.kind, func(t *testing.T) {
			cfgs := NewConfigs(tk.factories)
			conf := confmap.NewFromStringMap(map[string]any{
				"nop": map[string]any{
					"telemetry": map[string]any{
						"logger_provider": map[string]any{
							"processors": []any{
								map[string]any{
			...
			otelCfgs := cfgs.OtelConfigs()
			_, hasNop := otelCfgs[component.NewID(nopType)]
			assert.True(t, hasNop, "component with telemetry config should have an OTel config entry")
			_, hasDefault := otelCfgs[component.NewIDWithName(nopType, "default")]
			assert.False(t, hasDefault, "component without telemetry config should not have an OTel config entry")
		})
	}
}
