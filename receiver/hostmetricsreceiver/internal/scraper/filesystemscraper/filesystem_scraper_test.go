...
func TestScrapeMetrics(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			assertFileSystemUtilizationMetricValid(t, metrics.At(1), fileSystemUtilizationDescriptor)

			if isUnix() {
				assertFileSystemUsageMetricHasUnixSpecificStateLabels(t, metrics.At(0))
				assertFileSystemUtilizationMetricHasUnixSpecificStateLabels(t, metrics.At(1))

				assertFileSystemUsageMetricValid(t, metrics.At(2), fileSystemINodesUsageDescriptor)
				assertFileSystemUtilizationMetricValid(t, metrics.At(3), fileSystemINodesUtilizationDescriptor)
			}
		})
	}
}
...
func assertFileSystemUtilizationMetricValid(t *testing.T, metric pdata.Metric, descriptor pdata.MetricDescriptor) {
	internal.AssertDescriptorEqual(t, descriptor, metric.MetricDescriptor())
	assert.GreaterOrEqual(t, metric.DoubleDataPoints().Len(), 2)
	internal.AssertDoubleMetricLabelHasValue(t, metric, 0, stateLabelName, usedLabelValue)
	internal.AssertDoubleMetricLabelHasValue(t, metric, 1, stateLabelName, freeLabelValue)
}
...
func assertFileSystemUtilizationMetricHasUnixSpecificStateLabels(t *testing.T, metric pdata.Metric) {
	internal.AssertDoubleMetricLabelHasValue(t, metric, 2, stateLabelName, reservedLabelValue)
}
