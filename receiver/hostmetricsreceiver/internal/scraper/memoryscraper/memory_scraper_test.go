...
func TestScrapeMetrics(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			assertMemoryUtilizationMetricValid(t, metrics.At(1), memoryUtilizationDescriptor)
...
func assertMemoryUtilizationMetricValid(t *testing.T, metric pdata.Metric, descriptor pdata.MetricDescriptor) {
	internal.AssertDescriptorEqual(t, descriptor, metric.MetricDescriptor())
	assert.GreaterOrEqual(t, metric.DoubleDataPoints().Len(), 2)
	internal.AssertDoubleMetricLabelHasValue(t, metric, 0, stateLabelName, usedStateLabelValue)
	internal.AssertDoubleMetricLabelHasValue(t, metric, 1, stateLabelName, freeStateLabelValue)
}
...
func assertMemoryUtilizationMetricHasLinuxSpecificStateLabels(t *testing.T, metric pdata.Metric) {
	internal.AssertDoubleMetricLabelHasValue(t, metric, 2, stateLabelName, bufferedStateLabelValue)
	internal.AssertDoubleMetricLabelHasValue(t, metric, 3, stateLabelName, cachedStateLabelValue)
	internal.AssertDoubleMetricLabelHasValue(t, metric, 4, stateLabelName, slabReclaimableStateLabelValue)
	internal.AssertDoubleMetricLabelHasValue(t, metric, 5, stateLabelName, slabUnreclaimableStateLabelValue)
}
