...
func scrapeAndAppendCPUTimeMetric(metrics pdata.MetricSlice, startTime, now time.Time, handle processHandle) error {
...
func initializeCPUTimeMetric(metric pdata.Metric, startTime, now time.Time, times *cpu.TimesStat) {
...
func scrapeAndAppendMemoryUsageMetrics(metrics pdata.MetricSlice, now time.Time, handle processHandle) error {
...
func initializeMemoryUsageMetric(metric pdata.Metric, metricIntf metadata.MetricIntf, now time.Time, usage int64) {
...
func initializeMemoryUsageDataPoint(dataPoint pdata.IntDataPoint, now time.Time, usage int64) {
	dataPoint.SetTime(now)
	dataPoint.SetValue(usage)
}

func scrapeAndAppendDiskIOMetric(metrics pdata.MetricSlice, startTime, now time.Time, handle processHandle) error {
...
func initializeDiskIOMetric(metric pdata.Metric, startTime, now time.Time, io *process.IOCountersStat) {
...
func initializeDiskIODataPoint(dataPoint pdata.IntDataPoint, startTime, now time.Time, value int64, directionLabel string) {
