...
func TestScrape(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			if expectConntrackMetrics {
				assertNetworkConntrackMetricValid(t, metrics.At(expectedMetricCount+0), metadata.Metrics.SystemConntrackMax.New())
				assertNetworkConntrackMetricValid(t, metrics.At(expectedMetricCount+1), metadata.Metrics.SystemConntrackMax.New())
			}
