...
func TestScrape(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			if expectConntrackMetrics {
				assertNetworkConntrackMetricValid(t, metrics.At(0), metadata.Metrics.SystemConnectionsTableUsage.New())
				assertNetworkConntrackMetricValid(t, metrics.At(1), metadata.Metrics.SystemConnectionsTableLimit.New())
			}
