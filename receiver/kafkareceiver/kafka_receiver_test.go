...
func TestNewTracesReceiver_version_err(t *testing.T) {
	...
	r, err := newTracesReceiver(c, component.ComponentSettings{}, defaultTracesUnmarshalers(), consumertest.NewNop())
	assert.Error(t, err)
	assert.Nil(t, r)
}
...
func TestNewLogsReceiver_version_err(t *testing.T) {
	...
	r, err := newLogsReceiver(c, component.ComponentSettings{}, defaultLogsUnmarshalers(), consumertest.NewNop())
	assert.Error(t, err)
	assert.Nil(t, r)
}
