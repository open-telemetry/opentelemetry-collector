...
func (f *kafkaReceiverFactory) createTracesReceiver(
	...
	c := cfg.(*Config)
	r, err := newTracesReceiver(*c, componentSettings, f.tracesUnmarshalers, nextConsumer)
	if err != nil {
		return nil, err
	}
	return r, nil
...
func (f *kafkaReceiverFactory) createLogsReceiver(
	...
	c := cfg.(*Config)
	r, err := newLogsReceiver(*c, componentSettings, f.logsUnmarshalers, nextConsumer)
	if err != nil {
		return nil, err
	}
	return r, nil
