...
func TestCreateReceiverGeneralConfig(t *testing.T) {
	...
	componentSettings := component.ComponentSettings{Logger: zap.NewNop()}
	tReceiver, err := factory.CreateTracesReceiver(context.Background(), componentSettings, rCfg, nil)
	assert.NoError(t, err, "receiver creation failed")
	assert.NotNil(t, tReceiver, "receiver creation failed")

	mReceiver, err := factory.CreateMetricsReceiver(context.Background(), componentSettings, rCfg, nil)
	assert.Equal(t, err, componenterror.ErrDataTypeIsNotSupported)
	assert.Nil(t, mReceiver)
}
...
func TestCreateDefaultGRPCEndpoint(t *testing.T) {
	...
	componentSettings := component.ComponentSettings{Logger: zap.NewNop()}
	r, err := factory.CreateTracesReceiver(context.Background(), componentSettings, cfg, nil)
...
func TestCreateTLSThriftHTTPEndpoint(t *testing.T) {
	...
	params := component.ComponentSettings{Logger: zap.NewNop()}
