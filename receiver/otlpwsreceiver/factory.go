...
func (f *Factory) CreateTraceReceiver(
	...
	r, err := f.createReceiver(cfg, params.Logger)
	if err != nil {
		return nil, err
	}
...
func (f *Factory) createReceiver(cfg configmodels.Receiver, logger *zap.Logger) (*Receiver, error) {
	rCfg := cfg.(*Config)
	...
	if !ok {
		...
		var err error
		receiver, err = New(logger, rCfg.Name(), rCfg.Transport, rCfg.Endpoint, nil, nil)
		if err != nil {
			return nil, err
		}
