...
func (esc *ErrOrSinkConsumer) ConsumeTraces(ctx context.Context, td traces.Traces) error {
	esc.mu.Lock()
	defer esc.mu.Unlock()
...
func (esc *ErrOrSinkConsumer) ConsumeMetrics(ctx context.Context, md metrics.Metrics) error {
	esc.mu.Lock()
	defer esc.mu.Unlock()
