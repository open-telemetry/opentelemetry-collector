...
func initExponentialHistogramMetric(hm pmetric.Metric) {
	...
	hdp0.Positive().SetBucketCounts(pcommon.NewUInt64SliceFromRaw([]uint64{1, 1}))
	// negative index -1 and 0 are values -1/sqrt(2), -1 at scale 1
	hdp0.Negative().SetOffset(-1)
	hdp0.Negative().SetBucketCounts(pcommon.NewUInt64SliceFromRaw([]uint64{1, 1}))
	...
	hdp1.Positive().SetBucketCounts(pcommon.NewUInt64SliceFromRaw([]uint64{1, 1}))
