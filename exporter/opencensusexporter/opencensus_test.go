...
func TestSendTraces(t *testing.T) {
	...
	recv, err := rFactory.CreateTracesReceiver(context.Background(), componentSettings, rCfg, sink)
...
func TestSendMetrics(t *testing.T) {
	...
	rCfg.GRPCServerSettings.NetAddr.Endpoint = endpoint
	componentSettings := component.ComponentSettings{Logger: zap.NewNop()}
	recv, err := rFactory.CreateMetricsReceiver(context.Background(), componentSettings, rCfg, sink)
	assert.NoError(t, err)
	assert.NoError(t, recv.Start(context.Background(), componenttest.NewNopHost()))
	t.Cleanup(func() {
		assert.NoError(t, recv.Shutdown(context.Background()))
