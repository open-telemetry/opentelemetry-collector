...
func (f *Factory) OCAgentOptions(logger *zap.Logger, ocac *Config) ([]ocagent.ExporterOption, error) {
	...
	if ocac.TLSConfig.CertPemFile != "" {
		creds, err := credentials.NewClientTLSFromFile(ocac.TLSConfig.CertPemFile, "")
		if err != nil {
			return nil, &ocExporterError{
				code: errUnableToGetTLSCreds,
				msg:  fmt.Sprintf("OpenCensus exporter unable to read TLS credentials from pem file %q: %v", ocac.TLSConfig.CertPemFile, err),
			}
		}
