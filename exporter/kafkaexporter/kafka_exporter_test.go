...
func TestNewExporter_err_version(t *testing.T) {
	...
	texp, err := newTracesExporter(c, componenttest.NewTestExporterCreateSettings(t), tracesMarshalers())
	assert.Error(t, err)
	assert.Nil(t, texp)
}
...
func TestNewMetricsExporter_err_version(t *testing.T) {
	c := Config{ProtocolVersion: "0.0.0", Encoding: defaultEncoding}
	mexp, err := newMetricsExporter(c, componenttest.NewTestExporterCreateSettings(t), metricsMarshalers())
	assert.Error(t, err)
	assert.Nil(t, mexp)
}
...
func TestNewLogsExporter_err_version(t *testing.T) {
	c := Config{ProtocolVersion: "0.0.0", Encoding: defaultEncoding}
	mexp, err := newLogsExporter(c, componenttest.NewTestExporterCreateSettings(t), logsMarshalers())
	assert.Error(t, err)
	assert.Nil(t, mexp)
}
...
func TestNewExporter_err_auth_type(t *testing.T) {
	...
	assert.Contains(t, err.Error(), "failed to load TLS config")
	assert.Nil(t, mexp)
	lexp, err := newLogsExporter(c, componenttest.NewTestExporterCreateSettings(t), logsMarshalers())
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to load TLS config")
	assert.Nil(t, lexp)
