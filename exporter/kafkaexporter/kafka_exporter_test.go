...
func TestNewExporter_err_version(t *testing.T) {
	...
	texp, err := newTracesExporter(c, component.ComponentSettings{Logger: zap.NewNop()}, tracesMarshalers())
	assert.Error(t, err)
	assert.Nil(t, texp)
}
...
func TestNewMetricsExporter_err_version(t *testing.T) {
	c := Config{ProtocolVersion: "0.0.0", Encoding: defaultEncoding}
	mexp, err := newMetricsExporter(c, component.ComponentSettings{Logger: zap.NewNop()}, metricsMarshalers())
	assert.Error(t, err)
	assert.Nil(t, mexp)
}
...
func TestNewLogsExporter_err_version(t *testing.T) {
	c := Config{ProtocolVersion: "0.0.0", Encoding: defaultEncoding}
	mexp, err := newLogsExporter(c, component.ComponentSettings{Logger: zap.NewNop()}, logsMarshalers())
	assert.Error(t, err)
	assert.Nil(t, mexp)
}
...
func TestNewExporter_err_auth_type(t *testing.T) {
	...
	assert.Contains(t, err.Error(), "failed to load TLS config")
	assert.Nil(t, mexp)
	lexp, err := newLogsExporter(c, component.ComponentSettings{Logger: zap.NewNop()}, logsMarshalers())
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to load TLS config")
	assert.Nil(t, lexp)
