...
- `retry_on_failure`:  see [Retry on Failure](../exporterhelper/README.md#retry-on-failure) for the full set of available options.
  - `retryable_statuses` (default = `[429, 502, 503, 504]`): HTTP response status codes that are treated as retryable. The defaults match the [OTLP specification](https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/protocol/otlp.md#failures-1). Override this when the backend returns non-spec-compliant retryable codes — for example, when fronting the Collector with Cloudflare, which returns [HTTP 530](https://developers.cloudflare.com/support/troubleshooting/http-status-codes/cloudflare-5xx-errors/error-530) on upstream connectivity errors — or when running as a gateway and a code like `429` represents a non-retryable tenant limit that the gateway should surface to the client rather than retry (see #14228). Each code must be in the range 100-599. Note: `Retry-After` header handling remains scoped to HTTP 429 and 503 per the OTLP spec.
...
To extend the set of HTTP status codes that trigger retries (only do this for backends that emit non-spec-compliant retryable codes, such as Cloudflare returning HTTP 530 on upstream errors):
