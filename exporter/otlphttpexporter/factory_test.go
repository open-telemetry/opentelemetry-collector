...
func TestCreateDefaultConfig(t *testing.T) {
	...
	assert.Equalf(t, 30*time.Second, ocfg.ClientConfig.Timeout, "default timeout is 30 second")
	assert.Truef(t, ocfg.RetryConfig.Enabled, "default retry is enabled")
	assert.Equalf(t, 300*time.Second, ocfg.RetryConfig.MaxElapsedTime, "default retry MaxElapsedTime")
	assert.Equalf(t, 5*time.Second, ocfg.RetryConfig.InitialInterval, "default retry InitialInterval")
	assert.Equalf(t, 30*time.Second, ocfg.RetryConfig.MaxInterval, "default retry MaxInterval")
	assert.Truef(t, ocfg.QueueConfig.Enabled, "default sending queue is enabled")
