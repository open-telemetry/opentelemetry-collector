...
func (r *mockTracesReceiver) GetLastRequest() traces.Traces {
	r.mux.Lock()
	defer r.mux.Unlock()
...
func (r *mockLogsReceiver) GetLastRequest() logs.Logs {
	r.mux.Lock()
	defer r.mux.Unlock()
	return r.lastRequest
}
...
func (r *mockMetricsReceiver) GetLastRequest() metrics.Metrics {
	r.mux.Lock()
	defer r.mux.Unlock()
	return r.lastRequest
}
...
func startServerAndMakeRequest(t *testing.T, exp component.TracesExporter, td traces.Traces, ln net.Listener) {
	rcv, _ := otlpTracesReceiverOnGRPCServer(ln, false)
	defer rcv.srv.GracefulStop()
	// Ensure that initially there is no data in the receiver.
	assert.EqualValues(t, 0, atomic.LoadInt32(&rcv.requestCount))
