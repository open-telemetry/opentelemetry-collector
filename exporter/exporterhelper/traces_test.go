...
func TestTracesRequest(t *testing.T) {
	...
	traceErr := consumererror.NewTraces(errors.New("some error"), traces.New())
	assert.EqualValues(t, newTracesRequest(context.Background(), traces.New(), nil), mr.onError(traceErr))
}
...
func newTraceDataPusher(retError error) consumer.ConsumeTracesFunc {
	return func(ctx context.Context, td traces.Traces) error {
		return retError
	}
}
