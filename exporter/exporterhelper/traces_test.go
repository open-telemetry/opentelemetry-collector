...
func checkRecordedMetricsForTracesExporter(t *testing.T, tt obsreporttest.TestTelemetry, te exporter.TracesExporter, wantError error) {
	td := testdata.GenerateTraces(2)
	const numBatches = 7
	for i := 0; i < numBatches; i++ {
...
func generateTraceTraffic(t *testing.T, tracer trace.Tracer, te exporter.TracesExporter, numRequests int, wantError error) {
	td := ptrace.NewTraces()
...
func checkWrapSpanForTracesExporter(t *testing.T, sr *tracetest.SpanRecorder, tracer trace.Tracer, te exporter.TracesExporter, wantError error, numSpans int64) {
	const numRequests = 5
