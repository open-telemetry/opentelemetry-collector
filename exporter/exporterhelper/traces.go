...
func newTracesRequest(td ptrace.Traces, pusher consumer.ConsumeTracesFunc) request.Request {
...
func newTraceRequestUnmarshalerFunc(pusher consumer.ConsumeTracesFunc) request.Unmarshaler {
...
func tracesRequestMarshaler(req request.Request) ([]byte, error) {
...
func NewTracesExporter(
	...
	tc, err := consumer.NewTraces(func(ctx context.Context, td ptrace.Traces) error {
		req := intrequest.New(ctx, newTracesRequest(td, pusher))
		serr := be.send(req)
...
type TracesConverter interface {
	...
	RequestFromTraces(context.Context, ptrace.Traces) (request.Request, error)
}
...
func newTracesExporterWithObservability(obsrep *ObsReport, nextSender *senderWrapper) requestSender {
	return &tracesExporterWithObservability{obsrep: obsrep, nextSender: nextSender}
}

func (tewo *tracesExporterWithObservability) send(req *intrequest.Request) error {
