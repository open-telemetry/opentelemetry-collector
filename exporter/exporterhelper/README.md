...
- `requests`: number of incoming export calls. Each call counts as 1 regardless of its payload size. With
  the `requests` sizer, `min_size` and `max_size` have no effect: a batch is never split, and every request
  that arrives within `flush_timeout` is merged into a single outgoing export call;
- `items`: number of the smallest parts of each signal (spans, metric data points, log records);
- `bytes`: the size of serialized data in bytes (the least performant option).
