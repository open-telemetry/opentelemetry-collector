...
func TestMergeProfiles(t *testing.T) {
	...
	res, err := pr1.MergeSplit(context.Background(), map[request.SizerType]int64{request.SizerTypeItems: 0}, pr2)
...
func TestMergeProfilesInvalidInput(t *testing.T) {
	pr2 := newProfilesRequest(testdata.GenerateProfiles(3))
	_, err := pr2.MergeSplit(context.Background(), map[request.SizerType]int64{request.SizerTypeItems: 0}, &requesttest.FakeRequest{Items: 1})
	require.Error(t, err)
}
...
func TestMergeSplitManySmallProfiles(t *testing.T) {
	...
	for range 1000 {
		pr2 := newProfilesRequest(testdata.GenerateProfiles(10))
		res, _ := merged[len(merged)-1].MergeSplit(context.Background(), map[request.SizerType]int64{request.SizerTypeItems: 10000}, pr2)
	...
	for b.Loop() {
		...
		for range 1000 {
			...
			res, _ := merged[len(merged)-1].MergeSplit(
				context.Background(),
				map[request.SizerType]int64{request.SizerTypeBytes: int64(profilesMarshaler.ProfilesSize(testdata.GenerateProfiles(11000)))},
				pr2,
			)
			merged = append(merged[0:len(merged)-1], res...)
		}
...
func BenchmarkSplittingBasedOnByteSizeManyProfilesSlightlyAboveLimit(b *testing.B) {
	...
	for b.Loop() {
		...
		for range 10 {
			...
			res, _ := merged[len(merged)-1].MergeSplit(
				context.Background(),
				map[request.SizerType]int64{request.SizerTypeBytes: int64(profilesMarshaler.ProfilesSize(testdata.GenerateProfiles(10000)))},
				pr2,
			)
			assert.Len(b, res, 2)
			merged = append(merged[0:len(merged)-1], res...)
...
func BenchmarkSplittingBasedOnByteSizeHugeProfiles(b *testing.B) {
	...
	for b.Loop() {
		...
		res, _ := merged[len(merged)-1].MergeSplit(
			context.Background(),
			map[request.SizerType]int64{request.SizerTypeBytes: int64(profilesMarshaler.ProfilesSize(testdata.GenerateProfiles(10010)))},
			pr2,
		)
		merged = append(merged[0:len(merged)-1], res...)
