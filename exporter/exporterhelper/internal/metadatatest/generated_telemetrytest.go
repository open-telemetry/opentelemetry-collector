...
func AssertEqualExporterQueueBatchSize(t *testing.T, tt *componenttest.Telemetry, dps []metricdata.HistogramDataPoint[int64], opts ...metricdatatest.Option) {
	want := metricdata.Metrics{
		Name:        "otelcol_exporter_queue_batch_size",
		Description: "Size of the batches added to the retry queue (in bytes). [alpha]",
		Unit:        "{By}",
	...
	got, err := tt.GetMetric("otelcol_exporter_queue_batch_size")
	require.NoError(t, err)
	metricdatatest.AssertEqual(t, want, got, opts...)
}
