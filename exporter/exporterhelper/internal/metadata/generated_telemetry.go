...
func NewTelemetryBuilder(settings component.TelemetrySettings, options ...TelemetryBuilderOption) (*TelemetryBuilder, error) {
	...
	builder.ExporterDroppedLogRecords, err = builder.meter.Int64Counter(
		...
		metric.WithDescription("Number of log records intentionally dropped by the exporter (e.g. unsupported data). At detailed telemetry level, includes attribute: exporter.dropped.reason. [Alpha]"),
		metric.WithUnit("{record}"),
	)
	errs = errors.Join(errs, err)
	builder.ExporterDroppedMetricPoints, err = builder.meter.Int64Counter(
		"otelcol_exporter_dropped_metric_points",
		metric.WithDescription("Number of metric points intentionally dropped by the exporter (e.g. unsupported temporality). At detailed telemetry level, includes attribute: exporter.dropped.reason. [Alpha]"),
	...
	builder.ExporterDroppedProfileSamples, err = builder.meter.Int64Counter(
		...
		metric.WithDescription("Number of profile samples intentionally dropped by the exporter. At detailed telemetry level, includes attribute: exporter.dropped.reason. [Development]"),
		metric.WithUnit("{sample}"),
	)
	errs = errors.Join(errs, err)
	builder.ExporterDroppedSpans, err = builder.meter.Int64Counter(
		"otelcol_exporter_dropped_spans",
		metric.WithDescription("Number of spans intentionally dropped by the exporter (e.g. unsupported data). At detailed telemetry level, includes attribute: exporter.dropped.reason. [Alpha]"),
