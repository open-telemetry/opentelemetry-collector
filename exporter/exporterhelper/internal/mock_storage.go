...
func NewMockStorageExtension(client storage.Client) storage.Extension {
	return &mockStorageExtension{
		client: client,
	}
}

type failingStorageExtension struct {
	component.StartFunc
	component.ShutdownFunc
	getClientError error
}

func (m *failingStorageExtension) GetClient(_ context.Context, _ component.Kind, _ component.ID, _ string) (storage.Client, error) {
	return nil, m.getClientError
}

func NewFailingStorageExtension(getClientError error) storage.Extension {
	return &failingStorageExtension{getClientError: getClientError}
}
...
// this is useful for testing the persistent queue behavior with a full disk
