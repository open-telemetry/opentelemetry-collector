...
func TestToStorageClient(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			storageID := component.NewIDWithName(component.MustType("file_storage"), strconv.Itoa(tC.storageIndex))

			var extensions = map[component.ID]component.Component{}
			for i := 0; i < tC.numStorages; i++ {
				extensions[component.NewIDWithName(component.MustType("file_storage"), strconv.Itoa(i))] = NewMockStorageExtension(tC.getClientError)
			}
			host := &mockHost{ext: extensions}
			ownerID := component.NewID(component.MustType("foo_exporter"))
...
func TestInvalidStorageExtensionType(t *testing.T) {
	storageID := component.NewIDWithName(component.MustType("extension"), "extension")
