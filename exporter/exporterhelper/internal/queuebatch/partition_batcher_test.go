...
func TestPartitionBatcher_NoSplit_MinThresholdZero_TimeoutDisabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ba := newPartitionBatcher("test", cfg, tt.sizer, newWorkerPool(tt.maxWorkers), sink.Export)
...
func TestPartitionBatcher_Shutdown(t *testing.T) {
	...
	sink := requesttest.NewSink()
	ba := newPartitionBatcher("test", cfg, request.NewItemsSizer(), newWorkerPool(2), sink.Export)
	require.NoError(t, ba.Start(context.Background(), componenttest.NewNopHost()))
