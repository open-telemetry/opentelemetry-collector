...
func TestTracesRequestsSizerInvalidInputType(t *testing.T) {
	tr := newTracesRequest(testdata.GenerateTraces(2))
	_, err := tr.MergeSplit(context.Background(), 0, request.SizerTypeRequests, newLogsRequest(testdata.GenerateLogs(1)))
	require.Error(t, err)
}
