...
func NewTraceExporter(
	...
	oce, err := createOTLPExporter(config, params.Logger)
	if err != nil {
		return nil, err
	}
...
func createOTLPExporter(config configmodels.Exporter, logger *zap.Logger) (*otlpExporter, error) {
	oCfg := config.(*Config)
	...
	for exporterIndex := 0; exporterIndex < numWorkers; exporterIndex++ {
		...
		// as we do now.
		exporter, serr := newExporter(oCfg, logger)
		if serr != nil {
...
func (oce *otlpExporter) pushTraceData(ctx context.Context, td pdata.Traces) (int, error) {
	...
	err := exporter.exportRequest(ctx, request)
