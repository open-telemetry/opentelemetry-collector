...
```shell
$ ./bin/$(go env GOOS)/otcol  --config ./examples/demo/otel-agent-config.yaml
...
```shell
$ make otcol
$ ./bin/$($GOOS)/otcol --config ./examples/demo/otel-collector-config.yaml
```
...
the `otcol`, the example application will stop exporting. If you run it again,
...
1. Run from sources:
```shell
$ GO111MODULE=on go run github.com/open-telemetry/opentelemetry-service/cmd/otcol --help
```
...
$ docker run \
    ...
    --volume $(pwd)/otel-collector-config.yaml:/conf/otcol-config.yaml \
    otcol \
    --config=/conf/otcol-config.yaml
```
