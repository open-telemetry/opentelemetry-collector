...
func TestFunctionalityDownloadFileHTTP(t *testing.T) {
	fp := newConfigurableHTTPProvider(HTTPScheme, confmaptest.NewProviderSettingsNopLogger())
...
func TestFunctionalityDownloadFileHTTPS(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.testName, func(t *testing.T) {
			fp := newConfigurableHTTPProvider(HTTPSScheme, confmaptest.NewProviderSettingsNopLogger())
...
func TestUnsupportedScheme(t *testing.T) {
	fp := New(HTTPScheme, confmaptest.NewProviderSettingsNopLogger())
	_, err := fp.Retrieve(context.Background(), "https://...", nil)
	assert.Error(t, err)
	assert.NoError(t, fp.Shutdown(context.Background()))

	fp = New(HTTPSScheme, confmaptest.NewProviderSettingsNopLogger())
...
func TestValidateProviderScheme(t *testing.T) {
	assert.NoError(t, confmaptest.ValidateProviderScheme(New(HTTPScheme, confmaptest.NewProviderSettingsNopLogger())))
}

func TestInvalidTransport(t *testing.T) {
	fp := New("foo", confmaptest.NewProviderSettingsNopLogger())
