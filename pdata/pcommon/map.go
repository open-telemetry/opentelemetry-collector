...
func NewMap() Map {
	...
	return Map(internal.NewMap(&orig, nil))
}

func newMap(orig *[]otlpcommon.KeyValue, parent internal.Parent[*[]otlpcommon.KeyValue]) Map {
	return Map(internal.NewMap(orig, parent))
}
...
func (m Map) PutEmptyMap(k string) Map {
	...
	val := internal.NewValue(valOrig, internal.Map(m).GetValueParent(k))
	return Map(internal.NewMap(&kvl.KvlistValue.Values, internal.ValueMap{Value: val}))
}
...
func (m Map) PutEmptySlice(k string) Slice {
	...
	val := internal.NewValue(valOrig, internal.Map(m).GetValueParent(k))
	return Slice(internal.NewSlice(&vl.ArrayValue.Values, internal.ValueSlice{Value: val}))
}
