...
func newSliceFromParent(parent internal.Parent[*[]otlpcommon.AnyValue]) Slice {
...
func (es Slice) At(ix int) Value {
	return Value(internal.NewValueFromParent(internal.Slice(es).GetValueParent(ix)))
}
...
func (es Slice) CopyTo(dest Slice) {
	...
	for i := range *es.getOrig() {
		newValueFromOrig(&(*es.getOrig())[i]).CopyTo(newValueFromOrig(&(*dest.getOrig())[i]))
	}
}
...
func (es Slice) FromRaw(rawSlice []any) error {
	...
	for ix, iv := range rawSlice {
		errs = multierr.Append(errs, newValueFromOrig(&origs[ix]).FromRaw(iv))
	}
	*es.getOrig() = origs
	return errs
}
