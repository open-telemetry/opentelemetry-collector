...
func (es ResourceLogsSlice) CopyTo(dest ResourceLogsSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			newResourceLogs((*es.getOrig())[i], es.getState()).CopyTo(newResourceLogs((*dest.getOrig())[i], dest.getState()))
		}
		return
	}
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		newResourceLogs((*es.getOrig())[i], es.getState()).CopyTo(newResourceLogs(wrappers[i], dest.getState()))
	}
