...
func (es ScopeLogsSlice) CopyTo(dest ScopeLogsSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			newScopeLogs((*es.getOrig())[i], es.getState()).CopyTo(newScopeLogs((*dest.getOrig())[i], dest.getState()))
		}
		return
	}
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		newScopeLogs((*es.getOrig())[i], es.getState()).CopyTo(newScopeLogs(wrappers[i], dest.getState()))
	}
