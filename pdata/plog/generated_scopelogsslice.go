...
type pScopeLogsSlice struct {
	orig   *[]*otlplogs.ScopeLogs
	state  *internal.State
	parent ResourceLogs
}

func newScopeLogsSlice(orig *[]*otlplogs.ScopeLogs, parent ResourceLogs) ScopeLogsSlice {
	return ScopeLogsSlice{&pScopeLogsSlice{
		orig:   orig,
...
func (es ScopeLogsSlice) getOrig() *[]*otlplogs.ScopeLogs {
	if *es.state == internal.StateDirty {
		es.orig, es.state = es.parent.refreshScopeLogsOrigState()
	}
	return es.orig
}
...
func NewScopeLogsSlice() ScopeLogsSlice {
	...
	return ScopeLogsSlice{&pScopeLogsSlice{orig: &orig, state: &state}}
}
...
func (es ScopeLogsSlice) CopyTo(dest ScopeLogsSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			...
			destScopeLogs := ScopeLogs{&pScopeLogs{orig: (*dest.orig)[i], state: &exclState}}
