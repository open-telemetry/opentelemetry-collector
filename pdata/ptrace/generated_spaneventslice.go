...
func newSpanEventSliceFromElementOrig(elOrig *otlptrace.Span_Event) SpanEventSlice {
	orig := []*otlptrace.Span_Event{elOrig}
...
func (es SpanEventSlice) getChildOrig(i int) *otlptrace.Span_Event {
	return (*es.getOrig())[i]
}
...
func (es SpanEventSlice) At(i int) SpanEvent {
	return newSpanEventFromParent(es, i)
}
...
func (es SpanEventSlice) CopyTo(dest SpanEventSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			srcSpanEvent := newSpanEventFromOrig((*es.getOrig())[i])
			destSpanEvent := newSpanEventFromOrig((*dest.getOrig())[i])
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		srcSpanEvent := newSpanEventFromOrig((*es.getOrig())[i])
		destSpanEvent := newSpanEventFromOrig(wrappers[i])
		srcSpanEvent.CopyTo(destSpanEvent)
	}
