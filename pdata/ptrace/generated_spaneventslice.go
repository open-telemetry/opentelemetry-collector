...
func newSpanEventSliceFromOrig(orig *[]*otlptrace.Span_Event) SpanEventSlice {
	return SpanEventSlice{parent: newSpanFromEventsOrig(orig)}
}

func newSpanEventSliceFromParent(parent Span) SpanEventSlice {
	return SpanEventSlice{parent: parent}
}

func (es SpanEventSlice) getOrig() *[]*otlptrace.Span_Event {
	return es.parent.getEventsOrig()
}
...
func NewSpanEventSlice() SpanEventSlice {
	...
	return newSpanEventSliceFromOrig(&orig)
}
...
func (es SpanEventSlice) CopyTo(dest SpanEventSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			...
			destSpanEvent := SpanEvent{&pSpanEvent{orig: (*dest.getOrig())[i],
				state: &exclState}}
