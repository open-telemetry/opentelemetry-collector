...
func BenchmarkTracesUsage(b *testing.B) {
	logs := NewTraces()
	internal.Traces(logs).SetOrig(&otlpcollectortrace.ExportTraceServiceRequest{
		ResourceSpans: *generateTestResourceSpansSlice().getOrig(),
	})
