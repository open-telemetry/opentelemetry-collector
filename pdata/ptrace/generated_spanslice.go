...
func newSpanSliceFromElementOrig(elOrig *otlptrace.Span) SpanSlice {
	orig := []*otlptrace.Span{elOrig}
...
func (es SpanSlice) getChildOrig(i int) *otlptrace.Span {
	return (*es.getOrig())[i]
}
...
func (es SpanSlice) CopyTo(dest SpanSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			srcSpan := newSpanFromOrig((*es.getOrig())[i])
			destSpan := newSpanFromOrig((*dest.getOrig())[i])
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		srcSpan := newSpanFromOrig((*es.getOrig())[i])
		destSpan := newSpanFromOrig(wrappers[i])
		srcSpan.CopyTo(destSpan)
	}
