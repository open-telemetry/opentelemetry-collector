...
func (es SpanLinkSlice) CopyTo(dest SpanLinkSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			newSpanLink((*es.getOrig())[i], es.getState()).CopyTo(newSpanLink((*dest.getOrig())[i], dest.getState()))
		}
		return
	}
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		newSpanLink((*es.getOrig())[i], es.getState()).CopyTo(newSpanLink(wrappers[i], dest.getState()))
	}
