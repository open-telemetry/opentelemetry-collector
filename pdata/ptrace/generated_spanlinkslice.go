...
func newSpanLinkSliceFromElementOrig(elOrig *otlptrace.Span_Link) SpanLinkSlice {
	orig := []*otlptrace.Span_Link{elOrig}
...
func (es SpanLinkSlice) getChildOrig(i int) *otlptrace.Span_Link {
	return (*es.getOrig())[i]
}
...
func (es SpanLinkSlice) At(i int) SpanLink {
	return newSpanLinkFromParent(es, i)
}
...
func (es SpanLinkSlice) CopyTo(dest SpanLinkSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			srcSpanLink := newSpanLinkFromOrig((*es.getOrig())[i])
			destSpanLink := newSpanLinkFromOrig((*dest.getOrig())[i])
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		srcSpanLink := newSpanLinkFromOrig((*es.getOrig())[i])
		destSpanLink := newSpanLinkFromOrig(wrappers[i])
		srcSpanLink.CopyTo(destSpanLink)
	}
