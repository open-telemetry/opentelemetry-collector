...
type pSpanLinkSlice struct {
	orig   *[]*otlptrace.Span_Link
	state  *internal.State
	parent Span
}

func newSpanLinkSlice(orig *[]*otlptrace.Span_Link, parent Span) SpanLinkSlice {
	return SpanLinkSlice{&pSpanLinkSlice{
		orig:   orig,
...
func (es SpanLinkSlice) getOrig() *[]*otlptrace.Span_Link {
	if *es.state == internal.StateDirty {
		es.orig, es.state = es.parent.refreshLinksOrigState()
	}
	return es.orig
}
...
func NewSpanLinkSlice() SpanLinkSlice {
	...
	return SpanLinkSlice{&pSpanLinkSlice{orig: &orig, state: &state}}
}
...
func (es SpanLinkSlice) CopyTo(dest SpanLinkSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			...
			destSpanLink := SpanLink{&pSpanLink{orig: (*dest.orig)[i], state: &exclState}}
