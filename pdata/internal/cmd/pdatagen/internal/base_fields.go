...
const setTestSliceTemplate = `{{ if .isCommon -}}
	...
	New{{ .returnType }}FromParent({{ if .isBaseStructCommon }}W{{ else }}w{{ end }}rapped{{ .structName }}{{ .fieldName }}{{ "{" }}{{ .structName }}: tv}))
	{{- else if .parentUnspecified -}}
	fillTest{{ .returnType }}(new{{ .returnType }}FromParent(wrapped{{ .structName }}{{- .fieldName }}{{ "{" }}{{ .structName }}: tv}))
	{{- else -}}
	fillTest{{ .returnType }}(new{{ .returnType }}FromParent(tv))
	{{-	end }}`

const setTestValueTemplate = `{{ if .isCommon -}}
	internal.FillTest{{ .returnType }}(internal.New{{ .returnType }}FromParent(wrapped{{ .structName }}{{ .fieldName }}{{ "{" }}{{ .structName }}: tv}))
	{{- else if .parentUnspecified -}}
	fillTest{{ .returnType }}(new{{ .returnType }}FromParent(wrapped{{ .structName }}{{ .fieldName }}{{ "{" }}{{ .structName }}: tv}))
	{{- else -}}
	fillTest{{ .returnType }}(new{{ .returnType }}FromParent(tv))
...
func (ms {{ .structName }}) {{ .fieldName }}() {{ .packageName }}{{ .returnType }} {
	...
	return {{ .packageName }}{{ .returnType }}({{ if .isCommon }}internal.N{{ else }}n{{ end }}ew{{ .returnType }}FromParent(
	{{- if .isBaseStructCommon }}internal.W{{ else }}w{{ end }}rapped{{ .structName }}
...
func new{{ .structName }}From{{ .fieldName }}Orig(childOrig *{{ .returnOrigType }}) {{ .structName }} {
	return new{{ .structName }}FromOrig(&{{ .structOrigType }}{
		{{ .fieldName }}: *childOrig,
	})
}
{{- end }}`
...
func (ms {{ .structName }}) {{ .fieldName }}() {{ .packageName }}{{ .returnType }} {
	return {{ .packageName }}{{ .returnType }}(internal.New{{ .returnType }}FromParent(wrapped{{ .structName }}{{ .fieldName }}{{ "{" }}{{ .structName }}: ms}))
}`
...
const accessorsOneOfMessageParentTemplate = `func (es {{ .structName }}) get{{ .fieldName }}Orig() *{{ .originFieldPackageName }}.{{ .fieldName }} {
	return es.getOrig().Get{{ .originOneOfFieldName }}().(*{{ .originStructType }}).{{ .fieldName }}
}

func new{{ .structName }}From{{ .fieldName }}Orig(childOrig *{{ .originFieldPackageName }}.{{ .fieldName }}) {{ .structName }} {
	return new{{ .structName }}FromOrig(&{{ .structOrigType }}{
		{{ .originOneOfFieldName }}: &{{ .originStructType }}{
			{{ .fieldName }}: childOrig,
		},
	})
}`
...
func (ms {{ .structName }}) {{ .fieldName }}() {{ .returnType }} {
	_, ok := ms.getOrig().Get{{ .originOneOfFieldName }}().(*{{ .originStructType }})
	if !ok {
		return {{ .returnType }}{}
	}
	return new{{ .returnType }}FromParent(ms)
...
const setTestOneOfMessageTestTemplate = `	tv.getOrig().{{ .originOneOfFieldName }} = &{{ .originStructType }}
	...
	fillTest{{ .returnType }}(new{{ .fieldName }}FromParent(tv))`
