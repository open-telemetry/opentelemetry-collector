...
func (orig *ScopeProfiles) UnmarshalProtoUnsafe(buf []byte) error {
	return orig.unmarshalProto(buf, true)
}

func (orig *ScopeProfiles) unmarshalProto(buf []byte, unsafeUnmarshal bool) error {
	var err error
	var fieldNum int32
	...
	for pos < l {
		...
		case 2:
			...
			startPos := pos - length
			orig.Profiles = proto.GrowRepeated(orig.Profiles, buf, pos, fieldNum)
			orig.Profiles = append(orig.Profiles, NewProfile())
			err = orig.Profiles[len(orig.Profiles)-1].unmarshalProto(buf[startPos:pos], unsafeUnmarshal)
			if err != nil {
				return err
			}
