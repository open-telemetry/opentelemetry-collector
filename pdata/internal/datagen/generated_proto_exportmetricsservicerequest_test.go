...
func TestMarshalAndUnmarshalJSONMetricsUnknown(t *testing.T) {
	iter := json.BorrowIterator([]byte(`{"unknown": "string"}`))
	defer json.ReturnIterator(iter)
	dest := NewMetrics()
	UnmarshalJSONMetrics(dest, iter)
	require.NoError(t, iter.Error())
	assert.Equal(t, NewMetrics(), dest)
}

func TestMarshalAndUnmarshalJSONMetrics(t *testing.T) {
	for i, src := range generateTestValuesMetrics() {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			stream := json.BorrowStream(nil)
			defer json.ReturnStream(stream)
			MarshalJSONMetrics(src, stream)
...
func TestMarshalAndUnmarshalProtoMetricsUnknown(t *testing.T) {
	dest := NewMetrics()
	// message Test { required int64 field = 1313; } encoding { "field": "1234" }
	require.NoError(t, UnmarshalProtoMetrics(dest, []byte{0x88, 0x52, 0xD2, 0x09}))
	assert.Equal(t, NewMetrics(), dest)
}

func TestMarshalAndUnmarshalProtoMetrics(t *testing.T) {
	for i, src := range generateTestValuesMetrics(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoMetrics(src))
			gotSize := MarshalProtoSizedMetrics(src, buf)
			assert.Equal(t, len(buf), gotSize)

			dest := NewMetrics()
			require.NoError(t, UnmarshalProtoMetrics(dest, buf))
			assert.Equal(t, src, dest)
		})
	}
}

func TestMarshalAndUnmarshalProtoViaProtobufMetrics(t *testing.T) {
	for i, src := range generateTestValuesMetrics(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoMetrics(src))
			gotSize := MarshalProtoSizedMetrics(src, buf)
			...
			dest := NewMetrics()
			require.NoError(t, UnmarshalProtoMetrics(dest, goBuf))
			assert.Equal(t, src, dest)
		})
	}
}

func generateTestValuesMetrics() []*Metrics {
    return []*<no value>{
		{},
    }
}