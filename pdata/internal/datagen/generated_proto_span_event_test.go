...
func TestMarshalAndUnmarshalProtoSpanEventUnknown(t *testing.T) {
	...
	require.NoError(t, UnmarshalProtoSpanEvent(dest, []byte{0x88, 0x52, 0xD2, 0x09}))
	assert.Equal(t, NewSpanEvent(), dest)
}
...
func TestMarshalAndUnmarshalProtoSpanEvent(t *testing.T) {
	for i, src := range generateTestValuesSpanEvent(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoSpanEvent(src))
			gotSize := MarshalProtoSizedSpanEvent(src, buf)
			assert.Equal(t, len(buf), gotSize)

			dest := NewSpanEvent()
			require.NoError(t, UnmarshalProtoSpanEvent(dest, buf))
			assert.Equal(t, src, dest)
		})
	}
}
...
func TestMarshalAndUnmarshalProtoViaProtobufSpanEvent(t *testing.T) {
	for i, src := range generateTestValuesSpanEvent(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			dest := NewSpanEvent()
			require.NoError(t, UnmarshalProtoSpanEvent(dest, goBuf))
			assert.Equal(t, src, dest)
		})
	}
}

func generateTestValuesSpanEvent() []*SpanEvent {
    return []*<no value>{
		{},
    }
}