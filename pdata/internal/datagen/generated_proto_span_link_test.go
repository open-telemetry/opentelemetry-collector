...
func TestMarshalAndUnmarshalProtoSpanLinkUnknown(t *testing.T) {
	...
	require.NoError(t, UnmarshalProtoSpanLink(dest, []byte{0x88, 0x52, 0xD2, 0x09}))
	assert.Equal(t, NewSpanLink(), dest)
}
...
func TestMarshalAndUnmarshalProtoSpanLink(t *testing.T) {
	for i, src := range generateTestValuesSpanLink(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoSpanLink(src))
			gotSize := MarshalProtoSizedSpanLink(src, buf)
			assert.Equal(t, len(buf), gotSize)

			dest := NewSpanLink()
			require.NoError(t, UnmarshalProtoSpanLink(dest, buf))
			assert.Equal(t, src, dest)
		})
	}
}
...
func TestMarshalAndUnmarshalProtoViaProtobufSpanLink(t *testing.T) {
	for i, src := range generateTestValuesSpanLink(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			dest := NewSpanLink()
			require.NoError(t, UnmarshalProtoSpanLink(dest, goBuf))
			assert.Equal(t, src, dest)
		})
	}
}

func generateTestValuesSpanLink() []*SpanLink {
    return []*<no value>{
		{},
    }
}