...
func TestMarshalAndUnmarshalJSONAttributeUnitUnknown(t *testing.T) {
	iter := json.BorrowIterator([]byte(`{"unknown": "string"}`))
	defer json.ReturnIterator(iter)
	dest := NewAttributeUnit()
	UnmarshalJSONAttributeUnit(dest, iter)
	require.NoError(t, iter.Error())
	assert.Equal(t, NewAttributeUnit(), dest)
}

func TestMarshalAndUnmarshalJSONAttributeUnit(t *testing.T) {
	for i, src := range generateTestValuesAttributeUnit() {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			stream := json.BorrowStream(nil)
			defer json.ReturnStream(stream)
			MarshalJSONAttributeUnit(src, stream)
...
func TestMarshalAndUnmarshalProtoAttributeUnitUnknown(t *testing.T) {
	dest := NewAttributeUnit()
	// message Test { required int64 field = 1313; } encoding { "field": "1234" }
	require.NoError(t, UnmarshalProtoAttributeUnit(dest, []byte{0x88, 0x52, 0xD2, 0x09}))
	assert.Equal(t, NewAttributeUnit(), dest)
}

func TestMarshalAndUnmarshalProtoAttributeUnit(t *testing.T) {
	for i, src := range generateTestValuesAttributeUnit(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoAttributeUnit(src))
			gotSize := MarshalProtoSizedAttributeUnit(src, buf)
			assert.Equal(t, len(buf), gotSize)

			dest := NewAttributeUnit()
			require.NoError(t, UnmarshalProtoAttributeUnit(dest, buf))
			assert.Equal(t, src, dest)
		})
	}
}
...
func TestMarshalAndUnmarshalProtoViaProtobufAttributeUnit(t *testing.T) {
	for i, src := range generateTestValuesAttributeUnit(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			dest := NewAttributeUnit()
			require.NoError(t, UnmarshalProtoAttributeUnit(dest, goBuf))
			assert.Equal(t, src, dest)
		})
	}
}

func generateTestValuesAttributeUnit() []*AttributeUnit {
    return []*<no value>{
		{},
    }
}