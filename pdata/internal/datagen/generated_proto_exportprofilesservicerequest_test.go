...
func TestMarshalAndUnmarshalJSONProfilesUnknown(t *testing.T) {
	iter := json.BorrowIterator([]byte(`{"unknown": "string"}`))
	defer json.ReturnIterator(iter)
	dest := NewProfiles()
	UnmarshalJSONProfiles(dest, iter)
	require.NoError(t, iter.Error())
	assert.Equal(t, NewProfiles(), dest)
}

func TestMarshalAndUnmarshalJSONProfiles(t *testing.T) {
	for i, src := range generateTestValuesProfiles() {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			stream := json.BorrowStream(nil)
			defer json.ReturnStream(stream)
			MarshalJSONProfiles(src, stream)
...
func TestMarshalAndUnmarshalProtoProfilesUnknown(t *testing.T) {
	dest := NewProfiles()
	// message Test { required int64 field = 1313; } encoding { "field": "1234" }
	require.NoError(t, UnmarshalProtoProfiles(dest, []byte{0x88, 0x52, 0xD2, 0x09}))
	assert.Equal(t, NewProfiles(), dest)
}

func TestMarshalAndUnmarshalProtoProfiles(t *testing.T) {
	for i, src := range generateTestValuesProfiles(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoProfiles(src))
			gotSize := MarshalProtoSizedProfiles(src, buf)
			assert.Equal(t, len(buf), gotSize)

			dest := NewProfiles()
			require.NoError(t, UnmarshalProtoProfiles(dest, buf))
			assert.Equal(t, src, dest)
		})
	}
}

func TestMarshalAndUnmarshalProtoViaProtobufProfiles(t *testing.T) {
	for i, src := range generateTestValuesProfiles(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoProfiles(src))
			gotSize := MarshalProtoSizedProfiles(src, buf)
			...
			dest := NewProfiles()
			require.NoError(t, UnmarshalProtoProfiles(dest, goBuf))
			assert.Equal(t, src, dest)
		})
	}
}

func generateTestValuesProfiles() []*Profiles {
    return []*<no value>{
		{},
    }
}