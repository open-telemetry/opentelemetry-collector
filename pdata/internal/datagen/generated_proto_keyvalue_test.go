...
func TestMarshalAndUnmarshalJSONAttributeUnknown(t *testing.T) {
	iter := json.BorrowIterator([]byte(`{"unknown": "string"}`))
	defer json.ReturnIterator(iter)
	dest := NewAttribute()
	UnmarshalJSONAttribute(dest, iter)
	require.NoError(t, iter.Error())
	assert.Equal(t, NewAttribute(), dest)
}

func TestMarshalAndUnmarshalJSONAttribute(t *testing.T) {
	for i, src := range generateTestValuesAttribute() {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			stream := json.BorrowStream(nil)
			defer json.ReturnStream(stream)
			MarshalJSONAttribute(src, stream)
...
func TestMarshalAndUnmarshalProtoAttributeUnknown(t *testing.T) {
	dest := NewAttribute()
	// message Test { required int64 field = 1313; } encoding { "field": "1234" }
	require.NoError(t, UnmarshalProtoAttribute(dest, []byte{0x88, 0x52, 0xD2, 0x09}))
	assert.Equal(t, NewAttribute(), dest)
}

func TestMarshalAndUnmarshalProtoAttribute(t *testing.T) {
	for i, src := range generateTestValuesAttribute(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoAttribute(src))
			gotSize := MarshalProtoSizedAttribute(src, buf)
			assert.Equal(t, len(buf), gotSize)

			dest := NewAttribute()
			require.NoError(t, UnmarshalProtoAttribute(dest, buf))
			assert.Equal(t, src, dest)
		})
	}
}

func TestMarshalAndUnmarshalProtoViaProtobufAttribute(t *testing.T) {
	for i, src := range generateTestValuesAttribute(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoAttribute(src))
			gotSize := MarshalProtoSizedAttribute(src, buf)
			...
			dest := NewAttribute()
			require.NoError(t, UnmarshalProtoAttribute(dest, goBuf))
			assert.Equal(t, src, dest)
		})
	}
}

func generateTestValuesAttribute() []*Attribute {
    return []*<no value>{
		{},
    }
}