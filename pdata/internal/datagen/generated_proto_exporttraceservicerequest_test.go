...
func TestMarshalAndUnmarshalJSONTracesUnknown(t *testing.T) {
	iter := json.BorrowIterator([]byte(`{"unknown": "string"}`))
	defer json.ReturnIterator(iter)
	dest := NewTraces()
	UnmarshalJSONTraces(dest, iter)
	require.NoError(t, iter.Error())
	assert.Equal(t, NewTraces(), dest)
}

func TestMarshalAndUnmarshalJSONTraces(t *testing.T) {
	for i, src := range generateTestValuesTraces() {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			stream := json.BorrowStream(nil)
			defer json.ReturnStream(stream)
			MarshalJSONTraces(src, stream)
...
func TestMarshalAndUnmarshalProtoTracesUnknown(t *testing.T) {
	dest := NewTraces()
	// message Test { required int64 field = 1313; } encoding { "field": "1234" }
	require.NoError(t, UnmarshalProtoTraces(dest, []byte{0x88, 0x52, 0xD2, 0x09}))
	assert.Equal(t, NewTraces(), dest)
}

func TestMarshalAndUnmarshalProtoTraces(t *testing.T) {
	for i, src := range generateTestValuesTraces(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoTraces(src))
			gotSize := MarshalProtoSizedTraces(src, buf)
			assert.Equal(t, len(buf), gotSize)

			dest := NewTraces()
			require.NoError(t, UnmarshalProtoTraces(dest, buf))
			assert.Equal(t, src, dest)
		})
	}
}

func TestMarshalAndUnmarshalProtoViaProtobufTraces(t *testing.T) {
	for i, src := range generateTestValuesTraces(){
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			buf := make([]byte, SizeProtoTraces(src))
			gotSize := MarshalProtoSizedTraces(src, buf)
			...
			dest := NewTraces()
			require.NoError(t, UnmarshalProtoTraces(dest, goBuf))
			assert.Equal(t, src, dest)
		})
	}
}

func generateTestValuesTraces() []*Traces {
    return []*<no value>{
		{},
    }
}