...
func (ms Mapping) switchDictionary(src, dst ProfilesDictionary, mi *mergeIndex) error {
	if ms.FilenameStrindex() > 0 {
		...
		idx, err := mi.setString(dst.StringTable(), src.StringTable().At(int(ms.FilenameStrindex())))
		if err != nil {
