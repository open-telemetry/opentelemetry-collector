...
func TestResolveProfilesReferencesInlineKey(t *testing.T) {
	...
	// Simulate a KeyValue unmarshaled from a producer sending the key inline:
	// KeyStrindex is left at its proto3 default (0) since it was not serialized.
	mapOrig := internal.GetMapOrig(internal.MapWrapper(attrs))
	*mapOrig = append(*mapOrig, internal.KeyValue{
		Key: "service.name",
		Value: internal.AnyValue{
	...
	strVal, ok := kv.Value.Value.(*internal.AnyValue_StringValue)
	assert.True(t, ok)
	assert.Equal(t, "checkout", strVal.StringValue)
}
