...
func TestUnmarshalProfilesPreservesInlineKeys(t *testing.T) {
	profiles := NewProfiles()
	dict := profiles.Dictionary()
	dict.StringTable().Append("") // index 0
	...
	// Serialize the raw proto directly, bypassing convertProfilesToReferences,
	// to emulate a producer that sends attribute keys inline instead of
	// dictionary references.
	orig := profiles.getOrig()
	buf := make([]byte, orig.SizeProto())
	orig.MarshalProto(buf)

	unmarshaler := ProtoUnmarshaler{}
	restored, err := unmarshaler.UnmarshalProfiles(buf)
	require.NoError(t, err)

	serviceName, ok := restored.ResourceProfiles().At(0).Resource().Attributes().Get("service.name")
	assert.True(t, ok, "inline key should survive unmarshaling")
