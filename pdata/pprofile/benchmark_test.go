...
func init() {
	...
	profiles = map[string]*pprof.Profile{
		...
		// "large50percent": limitSamples(readPprof("profiles/large.pprof"), 0.5),
...
func benchmark(b *testing.B, profileName string, flavor string) {
	...
	for i := 0; i < b.N; i++ {
		...
		if (profileName == "simple" || profileName == "simpleWithLabelsAndTimestamps" || profileName == "jfr_1_1") && i == 0 && !printCache[profileName+flavor] {
			printCache[profileName+flavor] = true
			fmt.Printf("\n\nspot check [profile:%s; flavor: %s]\n", profileName, flavor)
			str := prettyPrintBuf(rop)
			fmt.Println(str)
			os.WriteFile(fmt.Sprintf("/tmp/%s-%s.yaml", profileName, flavor), []byte(str), 0644)
		}
