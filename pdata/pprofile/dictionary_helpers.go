...
func resolveKeyValueReferences(dict ProfilesDictionary, kvs []internal.KeyValue) {
	for i := range kvs {
		kv := &kvs[i]
		// Resolve key_ref if set. KeyStrindex == 0 is the proto3 default and
		// is never serialized, so it means an inline key was sent rather than
		// a reference to StringTable[0] (which must be "" per the OTLP spec).
		// Mirrors the string_value_strindex handling in resolveAnyValueReference.
		if kv.KeyStrindex > 0 {
