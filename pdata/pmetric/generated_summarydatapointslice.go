...
func newSummaryDataPointSliceFromElementOrig(elOrig *otlpmetrics.SummaryDataPoint) SummaryDataPointSlice {
	orig := []*otlpmetrics.SummaryDataPoint{elOrig}
...
func (es SummaryDataPointSlice) getChildOrig(i int) *otlpmetrics.SummaryDataPoint {
	return (*es.getOrig())[i]
}
...
func (es SummaryDataPointSlice) At(i int) SummaryDataPoint {
	return newSummaryDataPointFromParent(es, i)
}
...
func (es SummaryDataPointSlice) CopyTo(dest SummaryDataPointSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			srcSummaryDataPoint := newSummaryDataPointFromOrig((*es.getOrig())[i])
			destSummaryDataPoint := newSummaryDataPointFromOrig((*dest.getOrig())[i])
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		srcSummaryDataPoint := newSummaryDataPointFromOrig((*es.getOrig())[i])
		destSummaryDataPoint := newSummaryDataPointFromOrig(wrappers[i])
		srcSummaryDataPoint.CopyTo(destSummaryDataPoint)
	}
