...
func (es SummaryDataPointSlice) CopyTo(dest SummaryDataPointSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			newSummaryDataPoint((*es.getOrig())[i], es.getState()).CopyTo(newSummaryDataPoint((*dest.getOrig())[i], dest.getState()))
		}
		return
	}
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		newSummaryDataPoint((*es.getOrig())[i], es.getState()).CopyTo(newSummaryDataPoint(wrappers[i], dest.getState()))
	}
