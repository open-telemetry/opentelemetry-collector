...
type NumberDataPointSlice struct {
	parent internal.Parent[*[]*otlpmetrics.NumberDataPoint]
}
...
func (vp stubNumberDataPointSliceParent) EnsureMutability() {}

func (vp stubNumberDataPointSliceParent) GetChildOrig() *[]*otlpmetrics.NumberDataPoint {
	return vp.orig
}

var _ internal.Parent[*[]*otlpmetrics.NumberDataPoint] = (*stubNumberDataPointSliceParent)(nil)
...
func newNumberDataPointSliceFromParent(parent internal.Parent[*[]*otlpmetrics.NumberDataPoint]) NumberDataPointSlice {
	return NumberDataPointSlice{parent: parent}
}

func newNumberDataPointSliceFromElementOrig(elOrig *otlpmetrics.NumberDataPoint) NumberDataPointSlice {
	orig := []*otlpmetrics.NumberDataPoint{elOrig}
...
func (es NumberDataPointSlice) getChildOrig(i int) *otlpmetrics.NumberDataPoint {
	return (*es.getOrig())[i]
}
...
func (es NumberDataPointSlice) At(i int) NumberDataPoint {
	return newNumberDataPointFromParent(es, i)
}
...
func (es NumberDataPointSlice) CopyTo(dest NumberDataPointSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			srcNumberDataPoint := newNumberDataPointFromOrig((*es.getOrig())[i])
			destNumberDataPoint := newNumberDataPointFromOrig((*dest.getOrig())[i])
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		srcNumberDataPoint := newNumberDataPointFromOrig((*es.getOrig())[i])
		destNumberDataPoint := newNumberDataPointFromOrig(wrappers[i])
		srcNumberDataPoint.CopyTo(destNumberDataPoint)
	}
