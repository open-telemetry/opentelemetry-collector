...
func newResourceMetricsSliceFromElementOrig(elOrig *otlpmetrics.ResourceMetrics) ResourceMetricsSlice {
	orig := []*otlpmetrics.ResourceMetrics{elOrig}
...
func (es ResourceMetricsSlice) getChildOrig(i int) *otlpmetrics.ResourceMetrics {
	return (*es.getOrig())[i]
}
...
func (es ResourceMetricsSlice) At(i int) ResourceMetrics {
	return newResourceMetricsFromParent(es, i)
}
...
func (es ResourceMetricsSlice) CopyTo(dest ResourceMetricsSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			srcResourceMetrics := newResourceMetricsFromOrig((*es.getOrig())[i])
			destResourceMetrics := newResourceMetricsFromOrig((*dest.getOrig())[i])
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		srcResourceMetrics := newResourceMetricsFromOrig((*es.getOrig())[i])
		destResourceMetrics := newResourceMetricsFromOrig(wrappers[i])
		srcResourceMetrics.CopyTo(destResourceMetrics)
	}
