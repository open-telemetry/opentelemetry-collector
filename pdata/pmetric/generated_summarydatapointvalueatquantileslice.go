...
// Equal checks equality with another SummaryDataPointValueAtQuantileSlice
func (es SummaryDataPointValueAtQuantileSlice) Equal(val SummaryDataPointValueAtQuantileSlice) bool {
	if es.Len() != val.Len() {
		return false
	}
	for i := 0; i < es.Len(); i++ {
