...
func TestMetricsMutationCopyOnWrite(t *testing.T) {
	...
	newRS := newMS.ResourceMetrics().At(0)
	newRes := newRS.Resource()
	newSM := newRS.ScopeMetrics().At(0)
	newScope := newSM.Scope()
	newMetric := newSM.Metrics().At(0)
	newDP := newMetric.Sum().DataPoints().At(0)

	assert.EqualValues(t, internal.StateShared, *oldMS.getState())
	assert.EqualValues(t, internal.StateShared, *internal.Resource(oldRes).GetState())
	assert.EqualValues(t, internal.StateShared, *internal.InstrumentationScope(oldScope).GetState())
	assert.EqualValues(t, internal.StateShared, *oldMetric.getState())
	assert.EqualValues(t, internal.StateShared, *oldDP.getState())
	assert.EqualValues(t, internal.StateShared, *newMS.getState())
	assert.EqualValues(t, internal.StateShared, *internal.Resource(newRes).GetState())
	assert.EqualValues(t, internal.StateShared, *internal.InstrumentationScope(newScope).GetState())
	assert.EqualValues(t, internal.StateShared, *newMetric.getState())
	assert.EqualValues(t, internal.StateShared, *newDP.getState())
	...
	assert.EqualValues(t, internal.StateShared, *oldMetric.getState())
	assert.EqualValues(t, internal.StateShared, *oldDP.getState())
	assert.EqualValues(t, internal.StateExclusive, *newMS.getState())
	assert.EqualValues(t, internal.StateDirty, *internal.Resource(newRes).GetState())
	assert.EqualValues(t, internal.StateDirty, *internal.InstrumentationScope(newScope).GetState())
	assert.EqualValues(t, internal.StateExclusive, *newMetric.getState())
	assert.EqualValues(t, internal.StateExclusive, *newDP.getState())
}

func TestMetricsCopyToPassTheState(t *testing.T) {
	metrics := NewMetrics()
	fillTestResourceMetricsSlice(metrics.ResourceMetrics())
	metrics = metrics.AsShared()
	oldMetric := metrics.ResourceMetrics().At(0).ScopeMetrics().At(0).Metrics().At(0)
	assert.Equal(t, internal.StateShared, *oldMetric.getState())
	newMetric := NewMetric()
	newMetric.SetName("new-metric")
	newMetric.SetEmptySum()
	newMetric.CopyTo(oldMetric)
	assert.Equal(t, internal.StateExclusive, *oldMetric.getState())
	assert.Same(t, internal.Metrics(metrics).GetState(), oldMetric.getState())
	assert.Equal(t, "new-metric", oldMetric.Name())
	assert.Same(t, internal.Metrics(metrics).GetState(), oldMetric.Sum().getState())
	assert.NotSame(t, newMetric.Sum().getState(), oldMetric.Sum().getState())
}
