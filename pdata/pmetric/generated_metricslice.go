...
func (es MetricSlice) CopyTo(dest MetricSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			newMetric((*es.getOrig())[i], es.getState()).CopyTo(newMetric((*dest.getOrig())[i], dest.getState()))
		}
		return
	}
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		newMetric((*es.getOrig())[i], es.getState()).CopyTo(newMetric(wrappers[i], dest.getState()))
	}
