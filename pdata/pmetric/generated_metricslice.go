...
type MetricSlice interface {
	commonMetricSlice
	At(ix int) Metric
}

type MutableMetricSlice interface {
	commonMetricSlice
	At(ix int) MutableMetric
	EnsureCapacity(newCap int)
	AppendEmpty() MutableMetric
	Sort(less func(a, b MutableMetric) bool)
}

type commonMetricSlice interface {
	Len() int
	CopyTo(dest MutableMetricSlice)
	getOrig() *[]*otlpmetrics.Metric
}

type immutableMetricSlice struct {
	orig *[]*otlpmetrics.Metric
}

type mutableMetricSlice struct {
	immutableMetricSlice
}

func (es immutableMetricSlice) getOrig() *[]*otlpmetrics.Metric {
	return es.orig
}

func newImmutableMetricSlice(orig *[]*otlpmetrics.Metric) immutableMetricSlice {
	return immutableMetricSlice{orig}
}

func newMutableMetricSlice(orig *[]*otlpmetrics.Metric) mutableMetricSlice {
	return mutableMetricSlice{immutableMetricSlice{orig}}
}
...
func (es immutableMetricSlice) Len() int {
...
//	}
func (es immutableMetricSlice) At(ix int) Metric {
	return newImmutableMetric((*es.getOrig())[ix])
}
...
func (es immutableMetricSlice) CopyTo(dest MutableMetricSlice) {
	srcLen := es.Len()
	destCap := cap(*dest.getOrig())
	if srcLen <= destCap {
		(*dest.getOrig()) = (*dest.getOrig())[:srcLen:destCap]
		for i := range *es.getOrig() {
			newImmutableMetric((*es.getOrig())[i]).CopyTo(newMutableMetric((*dest.getOrig())[i]))
		}
		return
	}
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		newImmutableMetric((*es.getOrig())[i]).CopyTo(newMutableMetric(wrappers[i]))
	}
