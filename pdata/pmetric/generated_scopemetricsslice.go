...
type pScopeMetricsSlice struct {
	orig   *[]*otlpmetrics.ScopeMetrics
	state  *internal.State
	parent ResourceMetrics
}

func newScopeMetricsSlice(orig *[]*otlpmetrics.ScopeMetrics, parent ResourceMetrics) ScopeMetricsSlice {
	return ScopeMetricsSlice{&pScopeMetricsSlice{
		orig:   orig,
...
func (es ScopeMetricsSlice) getOrig() *[]*otlpmetrics.ScopeMetrics {
	if *es.state == internal.StateDirty {
		es.orig, es.state = es.parent.refreshScopeMetricsOrigState()
	}
	return es.orig
}
...
func (es ScopeMetricsSlice) getState() *internal.State {
	return es.state
}

func (es ScopeMetricsSlice) refreshElementOrigState(i int) (*otlpmetrics.ScopeMetrics, *internal.State) {
...
func NewScopeMetricsSlice() ScopeMetricsSlice {
	...
	return ScopeMetricsSlice{&pScopeMetricsSlice{orig: &orig, state: &state}}
}
...
func (es ScopeMetricsSlice) At(i int) ScopeMetrics {
	return newScopeMetrics((*es.getOrig())[i], es, i)
}
...
func (es ScopeMetricsSlice) CopyTo(dest ScopeMetricsSlice) {
	...
	if srcLen <= destCap {
		...
		for i := range *es.getOrig() {
			srcScopeMetrics := ScopeMetrics{&pScopeMetrics{orig: (*es.getOrig())[i], state: &exclState}}
			destScopeMetrics := ScopeMetrics{&pScopeMetrics{orig: (*dest.orig)[i], state: &exclState}}
	...
	for i := range *es.getOrig() {
		wrappers[i] = &origs[i]
		srcScopeMetrics := ScopeMetrics{&pScopeMetrics{orig: (*es.getOrig())[i], state: &exclState}}
		destScopeMetrics := ScopeMetrics{&pScopeMetrics{orig: wrappers[i], state: &exclState}}
		srcScopeMetrics.CopyTo(destScopeMetrics)
	}
	*dest.orig = wrappers
}
