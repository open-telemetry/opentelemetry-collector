...
func sizeProtoExponentialHistogramDataPointBuckets(orig *otlpmetrics.ExponentialHistogramDataPoint_Buckets) int {
	var n int
	_ = n
	var l int
	_ = l
	...
	for i := 0; i < len(orig.BucketCounts); i++ {
		l = internal.SizeProtouint64(&orig.BucketCounts[i])
		n += 1 + l + proto.Sov(uint64(l))
	}
	return n
}

func (ms ExponentialHistogramDataPointBuckets) marshalProto(buf []byte) (int, error) {
	return ms.orig.MarshalToSizedBuffer(buf)
}

func (ms ExponentialHistogramDataPointBuckets) unmarshalProto(buf []byte) error {
