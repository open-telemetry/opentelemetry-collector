...
func (col *Collector) getCfg() (cfg *config.Config, err error) {
...
func (col *Collector) reloadService(ctx context.Context) error {
	...
	for name, pipeline := range cfg.Pipelines {
		oldPipeline := col.service.config.Pipelines[name]
		if oldPipeline == nil {
			reload = false
			break
		}
	...
	if col.service != nil && reload {
		if err := col.service.Reload(ctx, cfg); err != nil {
			col.asyncErrorChannel <- err
			return err
		}
		col.service.config = cfg
		col.watchForUpdate()
		return nil
	} else {
		if col.service != nil {
...
func (col *Collector) watchForUpdate() {
