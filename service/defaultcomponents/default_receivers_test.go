...
func verifyReceiverLifecycle(t *testing.T, factory component.ReceiverFactory, getConfigFn getReceiverConfigFn) {
	...
	for _, createFn := range createFns {
		firstRcvr, err := createFn(ctx, componentSettings, getConfigFn())
		...
		secondRcvr, err := createFn(ctx, componentSettings, getConfigFn())
		require.NoError(t, err)
...
func wrapCreateLogsRcvr(factory component.ReceiverFactory) createReceiverFn {
	return func(ctx context.Context, componentSettings component.ComponentSettings, cfg config.Receiver) (component.Receiver, error) {
		return factory.CreateLogsReceiver(ctx, componentSettings, cfg, consumertest.NewNop())
	}
}

func wrapCreateMetricsRcvr(factory component.ReceiverFactory) createReceiverFn {
	return func(ctx context.Context, componentSettings component.ComponentSettings, cfg config.Receiver) (component.Receiver, error) {
		return factory.CreateMetricsReceiver(ctx, componentSettings, cfg, consumertest.NewNop())
	}
}

func wrapCreateTracesRcvr(factory component.ReceiverFactory) createReceiverFn {
	return func(ctx context.Context, componentSettings component.ComponentSettings, cfg config.Receiver) (component.Receiver, error) {
		return factory.CreateTracesReceiver(ctx, componentSettings, cfg, consumertest.NewNop())
	}
}
