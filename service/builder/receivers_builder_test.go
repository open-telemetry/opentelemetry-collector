...
func testReceivers(
	...
	assert.NoError(t, err)
	receivers, err := NewReceiversBuilder(zap.NewNop(), componenttest.TestApplicationStartInfo(), cfg, registries, pipelineProcessors, factories.Receivers).Build()
...
func TestReceiversBuilder_BuildCustom(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.dataType, func(t *testing.T) {
			...
			assert.NoError(t, err)
			pipelineProcessors, err := NewPipelinesBuilder(zap.NewNop(), componenttest.TestApplicationStartInfo(), cfg, registries, allExporters,
				factories.Processors).Build()
			assert.NoError(t, err)
			receivers, err := NewReceiversBuilder(zap.NewNop(), componenttest.TestApplicationStartInfo(), cfg, registries, pipelineProcessors, factories.Receivers).Build()
