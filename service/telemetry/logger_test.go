	...
	for _, tt := range tests {
		testCoreType := func(t *testing.T, wantCoreType any) {
			...
			if tt.disableTeeing == true {
				featuregate.GlobalRegistry().Set(disableConsoleLoggingWhenProviderEnabled.ID(), true)
				defer featuregate.GlobalRegistry().Set(disableConsoleLoggingWhenProviderEnabled.ID(), false)
			}
