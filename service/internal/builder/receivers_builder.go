...
func BuildReceivers(
	...
	rb := &receiversBuilder{logger.With(zap.String(zapKindKey, zapKindReceiver)), binInfo, config, builtPipelines, factories}

	receivers := make(Receivers)
	for _, cfg := range rb.config.Receivers {
		logger := rb.logger.With(zap.String(zapNameKey, cfg.Name()))
		rcv, err := rb.buildReceiver(context.Background(), logger, rb.binInfo, cfg)
		if err != nil {
...
func (rb *receiversBuilder) buildReceiver(ctx context.Context, logger *zap.Logger, binInfo component.BinaryInfo, config config.Receiver) (*builtReceiver, error) {
	...
	for dataType, pipelines := range pipelinesToAttach {
		...
		// this data type.
		err := rb.attachReceiverToPipelines(ctx, logger, binInfo, factory, dataType, config, rcv, pipelines)
		if err != nil {
			return nil, err
		}
	}
