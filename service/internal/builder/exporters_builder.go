...
func BuildExporters(
	...
	eb := &exportersBuilder{logger.With(zap.String(zapKindKey, zapKindLogExporter)), binInfo, config, factories}
	...
	for _, cfg := range eb.config.Exporters {
		...
		exp, err := eb.buildExporter(context.Background(), componentLogger, eb.binInfo, cfg, exporterInputDataTypes)
		if err != nil {
			return nil, err
		}
