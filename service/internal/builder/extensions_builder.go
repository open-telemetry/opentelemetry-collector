...
func BuildExtensions(
	...
	eb := &extensionsBuilder{logger.With(zap.String(zapKindKey, zapKindExtension)), binInfo, config, factories}
	...
	for _, extName := range eb.config.Service.Extensions {
		...
		ext, err := eb.buildExtension(componentLogger, eb.binInfo, extCfg)
		if err != nil {
			return nil, err
		}
...
func (eb *extensionsBuilder) buildExtension(logger *zap.Logger, binInfo component.BinaryInfo, cfg config.Extension) (*builtExtension, error) {
