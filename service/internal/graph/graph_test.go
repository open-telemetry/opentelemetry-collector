...
// testReloadPipelineAddition verifies that a new pipeline can be added
// via partial reload without affecting existing pipelines.
func testReloadPipelineAddition(t *testing.T) {
	t.Run("add_new_pipeline", func(t *testing.T) {
		rcvrCfgs := map[component.ID]component.Config{
			component.MustNewID("examplereceiver"): testcomponents.ExampleReceiverFactory.CreateDefaultConfig(),
		}
		procCfgs := map[component.ID]component.Config{
...
// testReloadPipelineRemoval verifies that a pipeline can be removed via partial reload.
// When pipelines share components (same receiver/exporter config), removing one pipeline
// causes shared components to be rebuilt because their pipeline membership changes.
func testReloadPipelineRemoval(t *testing.T) {
	t.Run("remove_pipeline", func(t *testing.T) {
		rcvrCfgs := map[component.ID]component.Config{
			component.MustNewID("examplereceiver"): testcomponents.ExampleReceiverFactory.CreateDefaultConfig(),
		}
		procCfgs := map[component.ID]component.Config{
...
func testReloadConnectorAddition(t *testing.T) {
	t.Run("add_connector_between_pipelines", func(t *testing.T) {
...
func testReloadConnectorRemoval(t *testing.T) {
	t.Run("remove_connector_between_pipelines", func(t *testing.T) {
...
func TestReloadExporterListChange(t *testing.T) {
	t.Run("with_internal_telemetry", func(t *testing.T) {
		setObsConsumerGateForTest(t, true)
		testReloadExporterListChange(t)
	})
	t.Run("without_internal_telemetry", func(t *testing.T) {
		setObsConsumerGateForTest(t, false)
		testReloadExporterListChange(t)
...
func testReloadExporterListChange(t *testing.T) {
	t.Run("add_exporter_to_pipeline", func(t *testing.T) {
...
func TestReloadSharedReceiverWithNewPipeline(t *testing.T) {
	t.Run("with_internal_telemetry", func(t *testing.T) {
		setObsConsumerGateForTest(t, true)
		testReloadSharedReceiverWithNewPipeline(t)
	})
	t.Run("without_internal_telemetry", func(t *testing.T) {
		setObsConsumerGateForTest(t, false)
		testReloadSharedReceiverWithNewPipeline(t)
...
func testReloadSharedReceiverWithNewPipeline(t *testing.T) {
	t.Run("new_pipeline_shares_receiver_with_existing", func(t *testing.T) {
...
func TestReloadReceiverAlreadyMarkedForRemoval(t *testing.T) {
	// Tests the skip path in markReceiversInAffectedPipelines where
	// a receiver is already in receiversToRemove.
	rcvrCfgs := map[component.ID]component.Config{
		component.MustNewID("examplereceiver"):                   struct{}{},
		component.MustNewIDWithName("examplereceiver", "second"): struct{}{},
	}
	...
	// Start with two pipelines, each with their own receiver.
	pipelineCfgs := pipelines.Config{
		pipeline.NewID(pipeline.SignalTraces): {
			Receivers:  []component.ID{component.MustNewID("examplereceiver")},
			Processors: []component.ID{component.MustNewID("exampleprocessor")},
	...
	// Remove a pipeline AND change processor config.
	// This causes the remaining pipeline to be in pipelinesAffected,
	// and the removed receiver is in receiversToRemove.
	newProcCfgs := map[component.ID]component.Config{
		component.MustNewID("exampleprocessor"): &struct{ Modified bool }{Modified: true},
	}
	newPipelineCfgs := pipelines.Config{
		pipeline.NewID(pipeline.SignalTraces): {
			Receivers:  []component.ID{component.MustNewID("examplereceiver")},
	...
	// Verify only one pipeline exists.
...
func TestHostWrapperReport(t *testing.T) {
	// Tests the HostWrapper.Report function by simulating a valid state transition.
	var reportedID *componentstatus.InstanceID
	var reportedStatus componentstatus.Status

	host := &Host{
		Reporter: status.NewReporter(
			func(id *componentstatus.InstanceID, event *componentstatus.Event) {
				reportedID = id
	...
	// Must follow valid state transitions: StatusNone -> StatusStarting
	event := componentstatus.NewEvent(componentstatus.StatusStarting)
	wrapper.Report(event)

	assert.Equal(t, instanceID, reportedID)
	assert.Equal(t, componentstatus.StatusStarting, reportedStatus)
}
