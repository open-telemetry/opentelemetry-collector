...
func (n *receiverNode) buildComponent(ctx context.Context,
	...
	case component.DataTypeTraces:
		var consumers []ctrace.Traces
		for _, next := range nexts {
			consumers = append(consumers, next.(ctrace.Traces))
		}
		n.Component, err = builder.CreateTraces(ctx, set, fanoutconsumer.NewTraces(consumers))
	case component.DataTypeMetrics:
		var consumers []cmetric.Metrics
		for _, next := range nexts {
			consumers = append(consumers, next.(cmetric.Metrics))
		}
...
func (n *processorNode) buildComponent(ctx context.Context,
	...
	case component.DataTypeTraces:
		n.Component, err = builder.CreateTraces(ctx, set, next.(ctrace.Traces))
	case component.DataTypeMetrics:
...
func (n *connectorNode) buildComponent(
	...
	case component.DataTypeTraces:
		...
		consumers := make(map[component.ID]ctrace.Traces, len(nexts))
		for _, next := range nexts {
			consumers[next.(*capabilitiesNode).pipelineID] = next.(ctrace.Traces)
	...
	case component.DataTypeMetrics:
		capability := consumer.Capabilities{MutatesData: false}
		consumers := make(map[component.ID]cmetric.Metrics, len(nexts))
		for _, next := range nexts {
			consumers[next.(*capabilitiesNode).pipelineID] = next.(cmetric.Metrics)
			capability.MutatesData = capability.MutatesData || next.Capabilities().MutatesData
		}
