...
func configureViews(level configtelemetry.Level) []config.View {
	if cached, ok := viewCache.Load(level); ok {
		return cached.([]config.View)
	}

	allConfigs := component.RegisteredMetricLevelConfigs()
	views := make([]config.View, 0, len(allConfigs))

	for _, cfg := range allConfigs {
		if level >= configtelemetry.Level(cfg.Level) {
			continue
		}
		selector := &config.ViewSelector{
			MeterName: stringPtr(cfg.MeterName),
		}
		if cfg.InstrumentName != "" {
			selector.InstrumentName = stringPtr(cfg.InstrumentName)
		}
		views = append(views, dropViewOption(selector))
	}

	viewCache.Store(level, views)
	return views
}
