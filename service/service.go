...
func configureViews(level configtelemetry.Level) []config.View {
	...
	if level < configtelemetry.LevelDetailed {
		...
		views = append(views,
			...
			dropViewOption(&config.ViewSelector{
				MeterName:      scope,
				InstrumentName: ptr("otelcol_exporter_retry_dropped_log_records"),
			}),
			dropViewOption(&config.ViewSelector{
				MeterName:      scope,
				InstrumentName: ptr("otelcol_exporter_retry_dropped_metric_points"),
			}),
			dropViewOption(&config.ViewSelector{
				MeterName:      scope,
				InstrumentName: ptr("otelcol_exporter_retry_dropped_spans"),
			}),
		)
	}
