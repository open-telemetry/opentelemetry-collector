...
type settings struct {
	...
	// BinaryInfo provides application start information.
...
func newService(settings *settings) (*service, error) {
	srv := &service{
		...
		binaryInfo:        settings.BinaryInfo,
...
func (srv *service) buildExtensions() error {
	var err error
	srv.builtExtensions, err = builder.BuildExtensions(srv.logger, srv.binaryInfo, srv.config, srv.factories.Extensions)
	if err != nil {
...
func (srv *service) buildPipelines() error {
	...
	var err error
	srv.builtExporters, err = builder.BuildExporters(srv.logger, srv.binaryInfo, srv.config, srv.factories.Exporters)
	if err != nil {
	...
	srv.builtPipelines, err = builder.BuildPipelines(srv.logger, srv.binaryInfo, srv.config, srv.builtExporters, srv.factories.Processors)
	if err != nil {
		return fmt.Errorf("cannot build pipelines: %w", err)
	}

	// Create receivers and plug them into the start of the pipelines.
	srv.builtReceivers, err = builder.BuildReceivers(srv.logger, srv.binaryInfo, srv.config, srv.builtPipelines, srv.factories.Receivers)
	if err != nil {
