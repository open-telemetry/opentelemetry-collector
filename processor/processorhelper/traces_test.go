...
func TestNewTracesProcessor_ProcessTraceError(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, want, tp.ConsumeTraces(context.Background(), traces.New()))
}
...
func TestNewTracesProcessor_ProcessTracesErrSkipProcessingData(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, nil, tp.ConsumeTraces(context.Background(), traces.New()))
}

func newTestTProcessor(retError error) ProcessTracesFunc {
	return func(_ context.Context, td traces.Traces) (traces.Traces, error) {
		return td, retError
	}
}
