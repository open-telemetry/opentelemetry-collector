...
func TestNewTraces(t *testing.T) {
	tp, err := NewTraces(context.Background(), processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), newTestTProcessor(nil))
...
func TestNewTraces_NilRequiredFields(t *testing.T) {
	_, err := NewTraces(context.Background(), processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), nil)
	assert.Error(t, err)
}

func TestNewTraces_ProcessTraceError(t *testing.T) {
	want := errors.New("my_error")
	tp, err := NewTraces(context.Background(), processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), newTestTProcessor(want))
	require.NoError(t, err)
...
func TestNewTraces_ProcessTracesErrSkipProcessingData(t *testing.T) {
	tp, err := NewTraces(context.Background(), processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), newTestTProcessor(ErrSkipProcessingData))
	require.NoError(t, err)
	assert.NoError(t, tp.ConsumeTraces(context.Background(), ptrace.NewTraces()))
}
...
func TestTracesConcurrency(t *testing.T) {
	...
	mp, err := NewTraces(context.Background(), processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), tracesFunc)
	require.NoError(t, err)
...
func TestTraces_RecordInOut(t *testing.T) {
	...
	tp, err := NewTraces(context.Background(), newSettings(tel), consumertest.NewNop(), mockAggregate)
	require.NoError(t, err)
...
func TestTraces_RecordIn_ErrorOut(t *testing.T) {
	...
	tel := componenttest.NewTelemetry()
	tp, err := NewTraces(context.Background(), newSettings(tel), consumertest.NewNop(), mockErr)
	require.NoError(t, err)
