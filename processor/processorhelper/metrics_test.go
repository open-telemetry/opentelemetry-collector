...
func TestNewMetrics(t *testing.T) {
	mp, err := NewMetrics(processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), newTestMProcessor(nil))
...
func TestNewMetrics_NilRequiredFields(t *testing.T) {
	_, err := NewMetrics(processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), nil)
	assert.Error(t, err)
}

func TestNewMetrics_ProcessMetricsError(t *testing.T) {
	want := errors.New("my_error")
	mp, err := NewMetrics(processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), newTestMProcessor(want))
	require.NoError(t, err)
...
func TestNewMetrics_ProcessMetricsErrSkipProcessingData(t *testing.T) {
	mp, err := NewMetrics(processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), newTestMProcessor(ErrSkipProcessingData))
	require.NoError(t, err)
	assert.NoError(t, mp.ConsumeMetrics(context.Background(), pmetric.NewMetrics()))
}
...
func TestMetricsConcurrency(t *testing.T) {
	...
	mp, err := NewMetrics(processortest.NewNopSettings(processortest.NopType), consumertest.NewNop(), metricsFunc)
	require.NoError(t, err)
	assert.NoError(t, mp.Start(context.Background(), componenttest.NewNopHost()))
...
func TestMetrics_RecordInOut(t *testing.T) {
	...
	mp, err := NewMetrics(newSettings(tel), consumertest.NewNop(), mockAggregate)
	require.NoError(t, err)
...
func TestMetrics_RecordIn_ErrorOut(t *testing.T) {
	...
	tel := componenttest.NewTelemetry()
	mp, err := NewMetrics(newSettings(tel), consumertest.NewNop(), mockErr)
	require.NoError(t, err)
