...
func TestNewUnhealthyProcessorFactory(t *testing.T) {
	...
	traces, err := factory.CreateTraces(context.Background(), NewNopSettingsWithType(factory.Type()), cfg, consumertest.NewNop())
	...
	metrics, err := factory.CreateMetrics(context.Background(), NewNopSettingsWithType(factory.Type()), cfg, consumertest.NewNop())
	require.NoError(t, err)
	...
	logs, err := factory.CreateLogs(context.Background(), NewNopSettingsWithType(factory.Type()), cfg, consumertest.NewNop())
	require.NoError(t, err)
