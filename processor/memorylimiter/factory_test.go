...
func TestCreateProcessor(t *testing.T) {
	...
	tp, err := factory.CreateTraceProcessor(context.Background(), component.ProcessorCreateParams{Logger: zap.NewNop()}, consumermock.Nil, cfg)
	assert.Nil(t, tp)
	assert.Error(t, err, "created processor with invalid settings")

	mp, err := factory.CreateMetricsProcessor(context.Background(), component.ProcessorCreateParams{Logger: zap.NewNop()}, consumermock.Nil, cfg)
	assert.Nil(t, mp)
	assert.Error(t, err, "created processor with invalid settings")
