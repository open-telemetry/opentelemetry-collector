...
func splitTraces(size int, src traces.Traces) traces.Traces {
	...
	src.ResourceSpans().RemoveIf(func(srcRs traces.ResourceSpans) bool {
		...
		srcRs.ScopeSpans().RemoveIf(func(srcIls traces.ScopeSpans) bool {
			// If we are done skip everything else.
			if totalCopiedSpans == size {
				return false
			}
			...
			srcIls.Spans().RemoveIf(func(srcSpan traces.Span) bool {
				// If we are done skip everything else.
				if totalCopiedSpans == size {
					return false
				}
...
// resourceSC calculates the total number of spans in the traces.ResourceSpans.
func resourceSC(rs traces.ResourceSpans) (count int) {
