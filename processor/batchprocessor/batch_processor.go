...
type batchTraces struct {
	...
	spanCount    int
	sizer        ptrace.TracesSizer
}

func newBatchTraces(nextConsumer consumer.Traces) *batchTraces {
	return &batchTraces{nextConsumer: nextConsumer, traceData: ptrace.NewTraces(), sizer: otlp.NewProtobufTracesMarshaler().(ptrace.TracesSizer)}
}
...
type batchMetrics struct {
	...
	dataPointCount int
	sizer          pmetric.MetricsSizer
}

func newBatchMetrics(nextConsumer consumer.Metrics) *batchMetrics {
	return &batchMetrics{nextConsumer: nextConsumer, metricData: pmetric.NewMetrics(), sizer: otlp.NewProtobufMetricsMarshaler().(pmetric.MetricsSizer)}
}
...
type batchLogs struct {
	...
	logData      plog.Logs
	logCount     int
	sizer        plog.LogsSizer
}

func newBatchLogs(nextConsumer consumer.Logs) *batchLogs {
	return &batchLogs{nextConsumer: nextConsumer, logData: plog.NewLogs(), sizer: otlp.NewProtobufLogsMarshaler().(plog.LogsSizer)}
}
