...
func TestBatchProcessorSpansDeliveredEnforceBatchSize(t *testing.T) {
	...
	componentSettings := component.ComponentSettings{Logger: zap.NewNop()}
	batcher, err := newBatchTracesProcessor(componentSettings, sink, cfg, configtelemetry.LevelBasic)
	require.NoError(t, err)
	require.NoError(t, batcher.Start(context.Background(), componenttest.NewNopHost()))
...
func TestBatchMetricProcessor_ReceivingData(t *testing.T) {
	...
	componentSettings := component.ComponentSettings{Logger: zap.NewNop()}
	batcher, err := newBatchMetricsProcessor(componentSettings, sink, &cfg, configtelemetry.LevelDetailed)
	require.NoError(t, err)
	require.NoError(t, batcher.Start(context.Background(), componenttest.NewNopHost()))
...
func TestBatchLogProcessor_ReceivingData(t *testing.T) {
	...
	componentSettings := component.ComponentSettings{Logger: zap.NewNop()}
	batcher, err := newBatchLogsProcessor(componentSettings, sink, &cfg, configtelemetry.LevelDetailed)
	require.NoError(t, err)
	require.NoError(t, batcher.Start(context.Background(), componenttest.NewNopHost()))
