...
func NewTelemetryBuilder(settings component.TelemetrySettings, options ...TelemetryBuilderOption) (*TelemetryBuilder, error) {
	...
	builder.ProcessorQueuebatchBatchSendSize, err = builder.meter.Int64Histogram(
		...
		metric.WithDescription("Number of items in the batch. Only recorded when batching is enabled. Only available on detailed level. [Development]"),
	...
	builder.ProcessorQueuebatchEnqueueSize, err = builder.meter.Int64Histogram(
		...
		metric.WithDescription("Number of items in a request offered to the queue, including rejected offers. Only available on detailed level. [Development]"),
		metric.WithUnit("{item}"),
		metric.WithExplicitBucketBoundaries([]float64{10, 25, 50, 75, 100, 250, 500, 750, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 20000, 30000, 50000, 100000}...),
	)
	errs = errors.Join(errs, err)
	builder.ProcessorQueuebatchEnqueueSizeBytes, err = builder.meter.Int64Histogram(
		...
		metric.WithDescription("Number of bytes in a request offered to the queue, including rejected offers. Only available on detailed level. [Development]"),
		metric.WithUnit("By"),
		metric.WithExplicitBucketBoundaries([]float64{128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1.048576e+06, 2.097152e+06, 4.194304e+06, 8.388608e+06, 1.6777216e+07}...),
	)
	errs = errors.Join(errs, err)
	...
	builder.ProcessorQueuebatchQueueCapacity, err = builder.meter.Int64ObservableGauge(
		...
		metric.WithDescription("Fixed capacity of the queue, expressed in units selected by the sizer attribute. [Development]"),
		metric.WithUnit("{unit}"),
	)
	errs = errors.Join(errs, err)
	builder.ProcessorQueuebatchQueueSize, err = builder.meter.Int64ObservableGauge(
		"otelcol_processor_queuebatch_queue_size",
		metric.WithDescription("Current size of the queue, expressed in units selected by the sizer attribute. [Development]"),
		metric.WithUnit("{unit}"),
	)
	errs = errors.Join(errs, err)
