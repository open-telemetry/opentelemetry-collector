	...
	for _, test := range tests {
		...
		for name, cfg := range cfg.Processors {
			t.Run(fmt.Sprintf("%s/%s", test.configName, name), func(t *testing.T) {
				...
				tp, tErr := factory.CreateTracesProcessor(context.Background(), componenttest.NewTestProcessorCreateSettings(t), cfg, consumertest.NewNop())
				// Not implemented error
				assert.NotNil(t, tErr)
				assert.Nil(t, tp)

				mp, mErr := factory.CreateMetricsProcessor(context.Background(), componenttest.NewTestProcessorCreateSettings(t), cfg, consumertest.NewNop())
