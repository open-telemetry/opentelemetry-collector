...
func TestLoadTLSClientConfigError(t *testing.T) {
	...
	_, err := tlsSetting.LoadTLSConfig(t.Context())
	assert.Error(t, err)
}
...
func TestLoadTLSClientConfig(t *testing.T) {
	...
	tlsCfg, err := tlsSetting.LoadTLSConfig(t.Context())
	require.NoError(t, err)
	assert.Nil(t, tlsCfg)
...
func TestSystemCertPool(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			c, err := serverConfig.LoadTLSConfig(t.Context())
			if test.wantErr != nil {
			...
			c, err = clientConfig.LoadTLSConfig(t.Context())
			if test.wantErr != nil {
...
func TestCurvePreferences(t *testing.T) {
	...
	for _, test := range tests {
		...
		config, err := tlsSetting.LoadTLSConfig(t.Context())
		if test.expectedErr == "" {
			require.NoError(t, err)
