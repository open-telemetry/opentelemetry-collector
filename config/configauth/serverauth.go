...
// ServerAuth is an Extension that can be used as an authenticator for the configauth.Authentication option.
// Authenticators are then included as part of OpenTelemetry Collector builds and can be referenced by their
// names from the Authentication configuration. Each ServerAuth is free to define its own behavior and configuration options,
...
// See ServerAuth.GrpcUnaryServerInterceptor.
...
// types for internal structs, making it easier to mock them in tests.
// See ServerAuth.GrpcStreamServerInterceptor.
