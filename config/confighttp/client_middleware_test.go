...
func TestClientMiddlewares(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create the client
			client, err := clientConfig.ToClient(context.Background(), componenttest.NewNopTelemetrySettings(), WithClientExtensions(extensions))
...
func TestClientMiddlewareErrors(t *testing.T) {
	...
	for _, tc := range httpTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := tc.config.ToClient(context.Background(), componenttest.NewNopTelemetrySettings(), WithClientExtensions(tc.extensions))
			require.Error(t, err)
