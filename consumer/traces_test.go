...
func TestDefaultTraces(t *testing.T) {
	cp, err := NewTraces(func(context.Context, traces.Traces) error { return nil })
	assert.NoError(t, err)
	assert.NoError(t, cp.ConsumeTraces(context.Background(), traces.New()))
...
func TestWithCapabilitiesTraces(t *testing.T) {
	cp, err := NewTraces(
		func(context.Context, traces.Traces) error { return nil },
...
func TestConsumeTraces(t *testing.T) {
	consumeCalled := false
	cp, err := NewTraces(func(context.Context, traces.Traces) error { consumeCalled = true; return nil })
	assert.NoError(t, err)
	assert.NoError(t, cp.ConsumeTraces(context.Background(), traces.New()))
...
func TestConsumeTraces_ReturnError(t *testing.T) {
	...
	cp, err := NewTraces(func(context.Context, traces.Traces) error { return want })
	assert.NoError(t, err)
	assert.Equal(t, want, cp.ConsumeTraces(context.Background(), traces.New()))
}
